...
-----------------------------------------------------------------------------------
function MasteryTrainingTest(pc, company, masteryTable, masteryName, noNeedReason, preCalcTotalCost)
	local isEnable = true;
	local needPoint = 0;
	---@type table|nil
	local reason = {};
	...
	-- 2. 마스터리 조건 체크
	isEnable, reason = IsEnableMasterRosterMastery(company, pc, mastery, masteryTable, noNeedReason, preCalcTotalCost);
...
-- 마스터리. PC 특성 체크 함수 - 
-----------------------------------------------------------------
function IsEnableByMasteryCheckType(pc, mastery, reason, noNeedReason, masteryChecker)
	local isEnable = true;
	...
	if masteryCheckType ~= 'All' then
		if mastery.FixedMastery then
			...
			local fixedMastery = GetWithoutError(pc, 'FixedMastery');
			if not fixedMastery and pc.RosterType == 'Beast' then
				fixedMastery = GetWithoutError(SafeIndex(pc, 'BeastType'), 'FixedMastery');
			end
		...
		elseif masteryCheckType == 'Job' then
			...
			if job and job.name and job.name ~= 'None' then
				local enableJobMasteryList = GetEnableJobMasteryList(pc.Object.Job);
				for _, key in ipairs(enableJobMasteryList) do
					if key == masteryTypeName then
						isEnableJobMastery = true;
						break;
					end
				end
				if masteryChecker == nil then
					masteryChecker = function(masteryName)
						if IsObject(pc) then
							return GetMasteryMastered(GetMastery(pc), masteryName) ~= nil;
						elseif GetIdspace(pc) == 'Monster' then
							local masteryLv = SafeIndex(pc, 'Masteries', masteryName);
				...
				local extraJobMasteryList = GetEnableExtraJobMasteryList(pc.Object, masteryChecker);
				for _, key in ipairs(extraJobMasteryList) do
					if key == masteryTypeName then
						isEnableJobMastery = true;
						break;
					end
...
function IsEnableMasterRosterMastery(company, pc, mastery, masteryTable, noNeedReason, preCalcTotalCost)
	...
	-- 3. TP 가능 테스트
	local totalCost = nil;
	if preCalcTotalCost then
		totalCost = preCalcTotalCost + mastery.Cost;
	else
		totalCost = GetCurrentMasteryCost(masteryTable);
	end
	...
	-- 7. 타입에 따른 조건 체크
	isEnable = isEnable and IsEnableByMasteryCheckType(pc, mastery, reason, noNeedReason);
	-- 8. 배타적 마스터리 체크
...
-- 특성판 복사 체크
-----------------------------------------------------------------
function MasteryBoardCopyTest(pc, company, from, to, itemCounter)
	local isEnable = true;
	local reason = {};
	local needItemCount = {};

	-- 1. 추출할 특성들만 뽑음
	local extractMasteryList = Linq.new(to)
		:select(function(kv) return kv[2] end)
		:where(function(mastery) return mastery.Lv > 0 and mastery.Category.EquipSlot ~= 'None' and mastery.ExtractItem ~= 'None' end)
		:where(function(mastery) return not GetMasteryMastered(from, mastery.name) end)
		:toList();
		
	-- 2. 추출 시 필요한 재료 여부 존재. 마스터리 코스트 만큼 필요.
	local needItemCount = {};
	...
	-- 3. 새로 복사할 특성들만 뽑음
	local copyMasteryList = Linq.new(from)
		:select(function(kv) return kv[2] end)
		:where(function(mastery) return mastery.Lv > 0 and mastery.Category.EquipSlot ~= 'None' and mastery.ExtractItem ~= 'None' end)
		:where(function(mastery) return not GetMasteryMastered(to, mastery.name) end)
		:toList();

	-- 4. 복사할 특성 보유 체크
	local notEnoughList = Linq.new(copyMasteryList)
		:where(function(mastery) return company.Mastery[mastery.name].Amount <= 0 end)
		:toList();

	copyMasteryList = Linq.new(copyMasteryList)
		:where(function(mastery) return company.Mastery[mastery.name].Amount > 0 end)
		:toList();
	
	return isEnable, reason, extractMasteryList, needItemCount, copyMasteryList, notEnoughList;
end
-----------------------------------------------------------------
-- 마스터리 카테고리 리스트 용
-----------------------------------------------------------------
function IsEnableMasterRosterMasteryForCategory(company, pc, mastery, noNeedReason)
	local isEnable = true;
	local reason = {};	
...
function IsEnableExtractRosterMastery(company, pc, mastery, masteryTable, itemCounter)
	...
	elseif not StringToBool(company.CheatState.NoUseManual) then
	...
	mbm:setPC(pc);
	mbm:setObject(pc.Object);
	mbm:loadMasteryTable(masteryTable);
...
----------------- 현재 배운 마스터리 체크 함수 --------------------
---------------------------------------------------------------------
---@param masteryTable table<string,class_Mastery>|nil
---@param masteryName string
---@return class_Mastery|nil
...
function GetMasteryMasteredImmuneWeather(masteryTable)
	local itemList = { 'Coat_Collector_Set', 'Coat_CollectorExpert_Set', 'Coat_CollectorMaster_Set', 'Coat_CollectorGrandMaster_Set'};
	return GetMasteryMasteredLikeWildLife(masteryTable) or GetMasteryMasteredList(masteryTable, itemList);
end
function GetMasteryMasteredImmuneTemperature(masteryTable)
	return GetMasteryMasteredLikeWildLife(masteryTable);
end
-- 살금 살금, 슬금 슬금, 소음 제어 장치
local s_likeStealthyFootstepsMastery = {'StealthyFootsteps', 'StealthyFootsteps_Beast', 'Module_NoiseControl'};
function GetMasteryMasteredLikeStealthyFootsteps(masteryTable)
	for _, masteryName in ipairs(s_likeStealthyFootstepsMastery) do
		local mastery = GetMasteryMastered(masteryTable, masteryName);
		if mastery then
			return mastery;
		end	
...
----------------- 버프 해제 면역 마스터리 체크 함수 --------------------
---------------------------------------------------------------------
function GetImmuneRemoveBuffMasteryList()
	-- 탐욕의 마녀
	return {'WitchOfGreed'};
end
function GetMasteryMasteredImmuneRemoveBuff(masteryTable)
	return GetMasteryMasteredList(masteryTable, GetImmuneRemoveBuffMasteryList());
end
---------------------------------------------------------------------
...
function GetCurrentMasteryCost(masteryTable)
	...
	if masteryTable then
		for key, mastery in pairs (masteryTable) do
			...
			local curMastery = mastertyList[mastery.name];
			if curMastery then
				if curMastery.Category.Type == 'Normal' or curMastery.Category.Type == 'Unique' or curMastery.Category.Type == 'Ability' or IsGradeMastery(curMastery.name) then
					if masteryLv and masteryLv > 0 then
...
function GetMasteryEventKey(owner)
	...
	if team == 'player' then
		...
		key = 'UnknownMasteryEvent'; -- 클라에서 후판단
	end
	return key;
end
------------------------------------------------------------------------
...
-- 무장연금 가방
function CP_BuffModifier_Amulet_BattleAlchemist_Set(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Base_MaxStack = buff.Base_MaxStack + mastery.ApplyAmount;
		end
	};
end
-- 어둠추적자 - 2 세트
---@param mastery class_Mastery
function CP_BuffModifier_DarkChaserSet2(mastery)
	local ret = {};
	local modifier = function(buff) 
		buff.Life = buff.Life + mastery.ApplyAmount;
		buff.Turn = buff.Turn + mastery.ApplyAmount;
	end

	for key, buffCls in pairs(GetClassList('Buff')) do
		---@type class_Buff
		local buffCls = buffCls;
		if buffCls.Group == mastery.BuffGroup.name then
			ret[key] = modifier;
		end
	end
	return ret;
...
-- 검은뱀 운동화
function CP_BuffModifier_Sneakers_Serpent_Black(mastery)
	local ret = {};
	local modifier = function(buff) 
		buff.Life = buff.Life + mastery.ApplyAmount;
		buff.Turn = buff.Turn + mastery.ApplyAmount;
...
-- 야수 결속
function CP_BuffModifier_BeastStrongBond(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Base_MaxStack = buff.Base_MaxStack + mastery.ApplyAmount;
		end
...
-- 야수 동화
function CP_BuffModifier_BeastAssimilation(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Base_MaxStack = buff.Base_MaxStack + mastery.ApplyAmount;
		end
...
-- 수풀 속의 그림자
function CP_BuffModifier_Stealth_Bush(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Life = buff.Life + mastery.ApplyAmount;
			buff.Turn = buff.Turn + mastery.ApplyAmount;
...
-- 외골격 경화
function CP_BuffModifier_HardExoskeleton(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Life = buff.Life + mastery.ApplyAmount3;
			buff.Turn = buff.Turn + mastery.ApplyAmount3;
		end
	};
end
-- 고성능 광학 위장 장치
function CP_BuffModifier_Module_EnhancedActiveCamouflage(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Life = buff.Life + mastery.ApplyAmount;
			buff.Turn = buff.Turn + mastery.ApplyAmount;
...
-- 고성능 광학 위장 장치
function CP_BuffModifier_Module_EnhancedBoosts_Speed(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Base_MaxStack = buff.Base_MaxStack + mastery.ApplyAmount;
		end
...
------------------------------------------------------------------------
-- BuffModifier Auto Registered Here
---------------------------------------------------------*BUFF_MODIFIERS
-- 화려한 폭주
---@param mastery class_Mastery
function CP_BuffModifier_Module_EnhancedMachineFury(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Life = buff.Life + mastery.ApplyAmount;
			buff.Turn = buff.Turn + mastery.ApplyAmount;
...
-- 고드름 창
---@param mastery class_Mastery
function CP_BuffModifier_IciclesMissile(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Base_MaxStack = buff.Base_MaxStack + mastery.ApplyAmount;
		end
...
-- 떨어지는 고드름
---@param mastery class_Mastery
function CP_BuffModifier_FallingIcicles(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.Base_MaxStack = buff.Base_MaxStack + mastery.ApplyAmount;
		end
...
-- 개량된 과출력 장치
---@param mastery class_Mastery
function CP_BuffModifier_Module_ImpovedOverdriveBooster(mastery)
	return {
		[mastery.Buff.name] = function(buff)
			buff.UseTurnStartCostAmount = buff.UseTurnStartCostAmount + mastery.ApplyAmount;
		end
	}
end
...
-----------------------------------------------------------------------
local g_gradeMasterySet = {
	Elite = true, Epic = true, Legend = true, Elite2 = true, Epic2 = true, Legend2 = true,
};
function IsGradeMastery(masteryName)
	return g_gradeMasterySet[masteryName] or false;
end
local g_monGradeMasteryMap = {
	Normal = { Elite = 'Elite', Epic = 'Epic', Legend = 'Legend' },
	HighRiskReturn = { Elite = 'Elite2', Epic = 'Epic2', Legend = 'Legend2' },
};
function GetNormalGradeMastery(gradeName)
	return SafeIndex(g_monGradeMasteryMap, 'Normal', gradeName);
end
...
function GetBaseMastery_Monster(target, monster)	-- 현 시점에서 targetster가 따로 있는게 아니니까 그냥 Object임
	...
	-- 2.2. 추가 직업 특성
	local extraJobMasteries = GetExtraJobMasteries(target, function(masteryName)
	...
	local gradeMasteryKey = 'Normal';
	-- 세컨드 윈드 - 성장하는 정예병
	if IsEnableSecondWind('HighRiskReturn', target) then
		gradeMasteryKey = 'HighRiskReturn';
	end
	local gradeMastery = SafeIndex(g_monGradeMasteryMap, gradeMasteryKey, target.Grade.name);
	if gradeMastery then
		list[gradeMastery] = 1;
	end
	return list;
end
function SetBasicMasteries(obj, isPc)
	...
	-- 5. 세트 아이템 특성
	if isPc then
		for k, v in pairs(GetItemSetMasteryList(obj)) do
			list[k] = v;
		end
	end
	return list;
end
function GetExtraJobMasteries(obj, masteryChecker)
	local list = {};
	local masteryList = GetClassList('Mastery');

	if masteryChecker == nil then
		masteryChecker = function(masteryName)
			return GetMasteryMastered(GetMastery(obj), masteryName) ~= nil;
		end;
	end

	-- 빛나는 재능 (StartJob의 직업 특성 추가)
	if masteryChecker('ShiningTalent') then
		...
		if startJobBasicMasteryName and startJobBasicMasteryName ~= 'None' then
			...
			if startJobBasicMastery and startJobBasicMastery ~= 'None' then
				table.insert(list, startJobBasicMastery);
			end
		end
	end
	-- 흉내내기 (무기 종류에 따른 직업 특성 추가)
	if masteryChecker('Munggo') then
		local enableEquip = GetWithoutError(obj, 'EnableEquipWeapon');
		if enableEquip and #enableEquip > 0 then
			local weaponType = enableEquip[1];
			local munggoMasteryCls = GetClassList('MunggoJobMastery')[weaponType];
			if munggoMasteryCls then
				local munggoJobCls = GetClassList('Job')[munggoMasteryCls.Job];
				local munggoJobBasicMasteryName = SafeIndex(munggoJobCls, 'BasicMastery');
				-- XXX 흉내내기
				for _, info in pairs(munggoMasteryCls.UpgradeMastery) do
					if masteryChecker(info.name) then
						munggoJobCls = GetClassList('Job')[info.Job];
						munggoJobBasicMasteryName = SafeIndex(munggoJobCls, 'BasicMastery');
					end
				end
				if munggoJobBasicMasteryName and munggoJobBasicMasteryName ~= 'None' then
					table.insert(list, munggoJobBasicMasteryName);
				end
			end
		end
	end
...
function GetItemSetMasteryList(obj)
	local list = {};
...
-- [MOD] 附加天赋效果统一阈值（解锁 / 生效 / 界面显示 三处共用）：
-- 数值 = 装备几个子天赋即视为该附加天赋效果（套装）成立。
-- 1=单天赋 / 2=双天赋 / 3=三天赋 / 4=四天赋（即原版需集齐全部）。
-- DATA 成品固定为 1；发布多版本 MOD 时由打包脚本替换此值。
XZJF_SetMasteryMinCount = 1;
function GetMasterySetTestSet()
	...
	local IterFunc = function(masterySetCls)
		...
		for _, needMastery in ipairs(masterySetCls.SubMasteryList) do
			ForceNewInsert(g_masterySetReference, needMastery.name, masterySetCls.name);
		end
...
function GetSetMastery(obj, masteryTable)
	...
	local enableDeveloingMasterySet = GetSystemConstant('EnableDevelopingMasterySet');
	
	local masterySetRef, remainSet = GetMasterySetTestSet();
	local masterySetSubCntMap = {};
	for _, mastery in pairs(masteryTable) do
		local masteryLv = SafeIndex(mastery, 'Lv');
		if masteryLv and masteryLv > 0 then
			for _, refSetMastery in ipairs(masterySetRef[mastery.name]) do
				masterySetSubCntMap[refSetMastery] = (masterySetSubCntMap[refSetMastery] or 0) + 1;
			end
		end
	end
	...
	for masteryName, subCnt in pairs(masterySetSubCntMap) do
		...
		if setCls and (not setCls.Developing or enableDeveloingMasterySet) then
			local isComplete = false;
			if isPlayerSide then
				isComplete = subCnt >= XZJF_SetMasteryMinCount;
			else
				isComplete = #setCls.SubMasteryList == subCnt;
			end
			if isComplete then
				table.bininsert(list, setCls, function(cls) return cls.Priority end);
			end
		end
	end
...
function GetSetMasteryLackList(pc, masteryTable, lackCount)
	...
	local masterySets = {};
	local enableDeveloingMasterySet = GetSystemConstant('EnableDevelopingMasterySet');
	local enableDLCSet = GetEnableDLCSet();
	...
	for _, masterySetCls in pairs(masterySetClsList) do
		(function()
			-- 개발 중 마스터리
			if masterySetCls.Developing and not enableDeveloingMasterySet then
				return;
			end
			-- DLC 체크
			local dlcName = SafeIndex(masterySetCls, 'DLC', 'name');
			if dlcName and dlcName ~= 'None' and not enableDLCSet[dlcName] then
				return;
			end
			-- 대상 마스터리 체크
			local acquiereMastery = masteryClsList[masterySetCls.name];
			...
			for _, propName in ipairs({'Mastery1', 'Mastery2', 'Mastery3', 'Mastery4'}) do
				...
				if propValue ~= 'None' then
					...
					-- 아예 착용이 불가능함
					local reason = {};
					if not IsEnableByMasteryCheckType(pc, needMastery, reason, true) then
						return;
					end
			...
			-- [MOD] 附加天赋效果界面显示条件放宽：
			-- 原版仅当恰好差 lackCount 个子天赋凑齐时才显示（如4子天赋套装需装3个）；
			-- 现改为已装备子天赋达到 XZJF_SetMasteryMinCount 个即显示在
			-- 附加天赋效果栏，与解锁 / 生效条件保持一致，方便查看哪些效果在生效。
			if hasCount >= XZJF_SetMasteryMinCount then
...
-----------------------------------------------------------------
-- Custom Cache Data (Don't change or remove this comment! including '--...--' part)
-----------------------------------------------------------------
-- 탁월한 조종
---@param obj class_Mastery
function Mastery_CustomCache_MasterContorolMachine(obj)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Scholar'}));
end

local ignoreMasteryCategoryList = { 'Equipment', 'EquipmentSet', 'System' };
function Mastery_CustomCache_MasteryCountByFunc(obj, testFunc)
	...
	return table.count(masteryTable, function(mastery)
		if mastery.Lv <= 0 then
			return false;
		elseif table.find(ignoreMasteryCategoryList, mastery.Category.name) then
			return false;
		end
...
function Mastery_CustomCache_MasteryCostByFunc(obj, testFunc)
	...
	local totalCost = 0;
	for _, mastery in pairs(masteryTable) do
		if mastery.Lv > 0 and not table.find(ignoreMasteryCategoryList, mastery.Category.name) and testFunc(mastery) then
...
function Mastery_CustomCache_ArchWitch(obj, arg)
	return Mastery_CustomCache_ESP(obj, arg);
end
function Mastery_CustomCache_ESP(obj, arg)
	return Mastery_CustomCache_MasteryCountByFunc(obj, function(mastery) return mastery.Type.CheckType == 'ESP'; end);
end
function Mastery_CustomCache_All(obj, arg)
	return Mastery_CustomCache_MasteryCountByFunc(obj, function(mastery) return mastery.Type.CheckType == 'All'; end);
end
...
function IsOriginalJobMastery(owner, mastery)
	if mastery.Type.CheckType ~= 'Job' then
		return false;
	end
	local enableJobMasteryList = string.split(GetHostClass(owner.Job).EnableJobMasteryFullCache, ',');
	for _, key in ipairs(enableJobMasteryList) do
		if key == mastery.Type.name then
			return true;
		end
	end
	return false;
...
function Mastery_CustomCache_RevealWildNature(obj, arg)
	local owner = GetMasteryOwner(obj);
	return Mastery_CustomCache_MasteryCountByFunc(obj, function(mastery) return mastery.Category.name == 'Beast' or mastery.Type.name == 'Beast' or IsOriginalJobMastery(owner, mastery) end);
end
...
-- 단단한 등딱지
function Mastery_CustomCache_HardCrabShell(obj, arg)
	local ret = {};
	for _, mType in ipairs({'Beast', 'Crammy', 'All'}) do
		ret[mType] = Mastery_CustomCache_MasteryCountByFunc(obj, function(mastery) return mastery.Type.name == mType; end);
	end
	ret.ESP = Mastery_CustomCache_MasteryCountByFunc(obj, function(mastery) return mastery.Type.CheckType == 'ESP'; end);
	return ret;
...
-- 혼연일체
function Mastery_CustomCache_UnitedSpiritAndBody(obj, arg)
	local ret = {};
	for _, mType in ipairs({'Spirit', 'Beast', 'All', 'Human'}) do
		ret[mType] = Mastery_CustomCache_MasteryCountByFunc(obj, function(mastery) return mastery.Type.name == mType; end);
	end
	return ret;
end
...
function Mastery_CustomCache_MagicianSwordMaster(obj, arg)
	local ret = {};
	for _, type in ipairs({ 'Swordsman', 'Mage', 'Swordmagician', 'All' }) do
		ret[type] = Mastery_CustomCache_MasteryCountByType(obj, Set.new({type}));
	end
	ret['ESP'] = Mastery_CustomCache_MasteryCountByFunc(obj, function(mastery) return mastery.Type.CheckType == 'ESP'; end);
	return ret;
...
-- 호환성 강화
function Mastery_CustomCache_Module_BlancerEnhancement(obj, arg)
	local ret = {};
	local maxCategories = nil;
	local maxValue = -1;
	for _, category in ipairs({ 'FrameModule', 'SupportModule', 'ComplementaryModule', 'SaftyModule', 'AIModule' }) do
		ret[category] = Mastery_CustomCache_MasteryCostByFunc(obj, function(mastery) return mastery.Category.name == category; end);
		if ret[category] > maxValue then
			maxValue = ret[category];
			maxCategories = {category};
	...
	local owner = GetMasteryOwner(obj);
	if owner.Info.name == 'Drone_Enhanced' then
		for category, value in pairs(ret) do
			ret[category] = value + 1;
		end
	end
	-- 호환성 최적화
	local mastery_SincereResearcher = GetMasteryMastered(GetMastery(owner), 'Module_SuperEnhanced');
	if mastery_SincereResearcher and maxCategories then
		for _, category in ipairs(maxCategories) do
			ret[category] = mastery_SincereResearcher.ApplyAmount * ret[category];
		end
...
-- 마법 다양성
function Mastery_CustomCache_MagicVariety(obj, arg)
	if IsClass(obj) then
		return 0;
	end
	local owner = GetMasteryOwner(obj);
	...
	local masteryTable = GetMastery(owner);
	for _, mastery in pairs(masteryTable) do
		if mastery.Lv > 0 and mastery.Type.CheckType ~= 'None' and not dupCheckSet[mastery.Type.name] then
			typeCount = typeCount + 1;
			dupCheckSet[mastery.Type.name] = true;
		end	
	end
	return typeCount;
end
-- 노련한 결투사
function Mastery_CustomCache_ExpertDuelist(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Warrior', 'WhiteKnight'}));
end
-- 검의 기사
function Mastery_CustomCache_KnightOfSword(obj, arg)
	local ret = {};
	ret['ExpertDuelist'] = Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Swordsman'}));
	ret['SwordsmanshipBook'] = Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Warrior', 'WhiteKnight'}));
	return ret;
end
-- 거대한 근육
function Mastery_CustomCache_MuscleBarbarian(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Warrior', 'Barbarian'}));
end
-- 오만의 마녀
function Mastery_CustomCache_WitchOfPride(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Mage'}));
end
-- @@의 룬
function Mastery_CustomCache_SubType(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({obj.SubType}));
end
-- 본성 각성
function Mastery_CustomCache_InstinctAwaken(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Nature'}));
end
-- 절연검, 엘류오트
function Mastery_CustomCache_Breaking(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Swordsman', 'Greatswordsman'}));
end
-- 핑크 혼
function Mastery_CustomCache_PinkHorn(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Mage', 'Swordmagician'}));
end
-- 교활한 손놀림
function Mastery_CustomCache_CunningHands(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Thief'}));
end
-- 아드레날린 촉진
function Mastery_CustomCache_AdrenalineRush(obj, arg)
	local ret = {};
	for _, type in ipairs({ 'Human', 'Thief', 'Rogue', 'All' }) do
		ret[type] = Mastery_CustomCache_MasteryCountByType(obj, Set.new({type}));
	end
	return ret;
end
-- 술법 강화
function Mastery_CustomCache_ReinforcedJutsu(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Shaman', 'Ninja'})) + Mastery_CustomCache_ESP(obj, arg);
end
-- 체술 강화
function Mastery_CustomCache_EnhancedTaijutsu(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Thief', 'Ninja'}));
end
-- 회복술 강화
function Mastery_CustomCache_HealJutsu(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Shaman', 'Ninja'})) + Mastery_CustomCache_ESP(obj, arg);
end
-- 암호 해킹
function Mastery_CustomCache_PasswordHacking(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Thief', 'Hacker'}));
end
-- 악랄한 강탈자
function Mastery_CustomCache_EvilRobber(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Thief', 'Rogue'}));
end
-- 살의
function Mastery_CustomCache_MurderousIntent(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Marauder', 'Slaughterer'}));
end
-- 대혼전
function Mastery_CustomCache_HeavyIntenseBattle(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Marauder', 'Slaughterer'}));
end
-- 검객 + 검호
function Mastery_CustomCache_SGS(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Swordsman', 'Greatswordsman'}));
end
-- 인간
function Mastery_CustomCache_Human(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Human'}));
end
-- 정령 교감
function Mastery_CustomCache_ElementalCommunion(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Human', 'Shaman'}));
end
-- 영리한 손놀림, 영리한 발놀림, 영리한 측면 공격
function Mastery_CustomCache_Job_Training(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Munggo', 'Training'}));
end
-- 황금 숨결
function Mastery_CustomCache_GoldBreath(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Draky'}));
end
-- 강철 비늘
function Mastery_CustomCache_IronScale(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Draky'}));
end
-- 영리한 야수
function Mastery_CustomCache_BeastAndNature(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Beast', 'Nature'}));
end
-- 압도하는 악력
function Mastery_CustomCache_BeastAndGene(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'Beast', 'Gene'}));
end
-- 신속한 모방 회피
function Mastery_CustomCache_AllTypeCount(obj, arg)
	return Mastery_CustomCache_MasteryCountByType(obj, Set.new({'All'}));
end
-- 약자 멸시
function Mastery_CustomCache_MinHPTarget(obj, arg)
	local o = GetMasteryOwner(obj);
	if not o then
		return '';
	end
	local minHPTarget = Linq.new(GetAllUnitInSight(o, true))
		:where(function(u) return u.HP < u.MaxHP and IsEnemy(o, u) end)
		:orderByAscending(function(u) return u.HP; end)
		:first();
	if minHPTarget then
		return GetObjKey(minHPTarget);
	else
		return '';
	end
end
...
-- MasteryBoardManager
-----------------------------------------------------------------
---@class MasteryBoardManager
---@field Target class_Pc|class_Monster|class_Machine
---@field MasteryTable table<string,class_Mastery>
---@field IgnoreMasteryLimit boolean
---@field TargetWrapper any
---@field SetMasteries any
---@field TP any
---@field MaxTP any
---@field Object any
---@field CategoryList any
---@field MasterySet any
---@field MasterySetRef any
MasteryBoardManager = {}
function MasteryBoardManager.new(pcInfo, masteryTable, ignoreMasteryLimit)
	local ret = {Target = pcInfo, TP = 0, MasteryTable = {}, SetMasteries = {}, IgnoreMasteryLimit = ignoreMasteryLimit};
...
function MasteryBoardManager.newDummyPc(self)
	...
	dummy:addCachedValue('MaxTP', function()
		if self.IgnoreMasteryLimit then
			return 9999;
		end
...
function MasteryBoardManager.buildSetMastery(self)
	...
	for setName, setInfo in pairs(self.MasterySet) do
		-- [MOD] 附加天赋效果组成条件放宽：
		-- 原版需集齐套装全部子天赋（RefCount == NeedCount）才将 Set 天赋计入
		-- 天赋板（影响机器负载/TP 等）；现改为已装备子天赋达到
		-- XZJF_SetMasteryMinCount 个即视为套装已组成，与战斗内 GetSetMastery
		-- 及界面显示判定保持一致。
		if setInfo.RefCount >= XZJF_SetMasteryMinCount then
...
function MasteryBoardManager._addMastery(self, mastery)
	...
	if refList then
		for _, refInfo in ipairs(refList) do
			...
			-- [MOD] 套装组成条件放宽：达到 XZJF_SetMasteryMinCount 个子天赋即触发重建套装列表
			if refInfo.Ref.RefCount >= XZJF_SetMasteryMinCount then
...
function MasteryBoardManager.isEnableAddMastery(self, masteryName)
	...
	local reason = {};
	if not IsEnableByMasteryCheckType(self.TargetWrapper, mastery, reason, true, function(masteryName) return self.MasteryTable[masteryName] ~= nil end) then
...
function CalculatedProperty_InfoRange_Sandstorm(obj)
	local owner = GetMasteryOwner(obj);
	if owner == nil then
		return CalculatedProperty_InfoRange(obj);
	end
	local mastery_WhiteSandstorm = GetMasteryMastered(GetMastery(owner), 'WhiteSandstorm');
	if mastery_WhiteSandstorm == nil then
		return CalculatedProperty_InfoRange(obj);
	end
	return mastery_WhiteSandstorm.Range;
end

function CalculatedProperty_InfoRange_HardshipBattle(obj)
	local owner = GetMasteryOwner(obj);
	if owner == nil then
		return CalculatedProperty_InfoRange(obj);
	end
	local mastery_FrenzyBarbarian = GetMasteryMastered(GetMastery(owner), 'FrenzyBarbarian');
	if mastery_FrenzyBarbarian == nil then
		return CalculatedProperty_InfoRange(obj);
	end
	return mastery_FrenzyBarbarian.Range;
end
-- 긴급 구조 프로그램
function CalculatedProperty_InfoRange_Module_EmergencyRescue(obj)
	local owner = GetMasteryOwner(obj);
	if owner == nil then
		return CalculatedProperty_InfoRange(obj);
	end
	local applyDist = obj.ApplyAmount;
	-- 향상된 긴급 구조 프로그램
	...
	return 'Sphere'..applyDist;
end
-- 자동 응징
function CalculatedProperty_InfoRange_Module_Retribution(obj)
	local owner = GetMasteryOwner(obj);
	if owner == nil then
		return CalculatedProperty_InfoRange(obj);
	end
	local applyDist = obj.ApplyAmount;
	-- 향상된 자동 응징
	local mastery_Module_EnhancedRetribution = GetMasteryMastered(GetMastery(owner), 'Module_EnhancedRetribution');
	if mastery_Module_EnhancedRetribution then
		applyDist = applyDist + mastery_Module_EnhancedRetribution.ApplyAmount;
	end
	return 'Sphere'..applyDist;
end
...
------------------------- 특성판 코드 CP -------------------------------------
-----------------------------------------------------------------------------------
function CalculatedProperty_MasteryCodeInverted(obj, arg)
	local inverted = {};
	if obj.name == 'Mastery' then
		-- 특성 코드는 값의 크기를 줄이기 위해, Type을 같이 봐야함
		for k, v in pairs(obj.Codes) do
			local type = v.Type;
			local code = v.Code;
			local typeMap = inverted[type];
			if typeMap == nil then
				typeMap = {};
				inverted[type] = typeMap;
			end
			typeMap[code] = k;
		end
	else
...
-----------------------------------------------------------------
-- 특성판 불러오기 선체크 (인간/야수와 기계는 특성/모듈 차이 때문에 완전히 호환이 안 되므로, 미리 선 체크함)
-----------------------------------------------------------------
function IsEnableImportMasteryCode(pcInfo, codeInfo)
	local isEnable = true;
	local reason = {};
	
	-- 특성과 모듈은 호환 불가능
	if (pcInfo.RosterType == 'Machine') ~= (codeInfo.RosterType == 'Machine') then
		isEnable = false;
		table.insert(reason, 'MasteryCode_MasteryModuleNotMatched');
		return isEnable, reason;
	end
...
-- 특성판 불러오기 체크
-----------------------------------------------------------------
function MasteryCodeImportTest(pcInfo, company, codeInfo, to, itemCounter)
	local isEnable = true;
	local reason = {};
	local needItemCount = {};
	
	local fromSet = Set.new(codeInfo.Mastery);
	local toSet = Linq.new(to)
		:select(function(kv) return kv[2] end)
		:where(function(mastery) return mastery.Lv > 0 and mastery.Category.EquipSlot ~= 'None' and mastery.ExtractItem ~= 'None' end)
		:select(function(mastery) return mastery.name end)
		:toSet();

	local basicMasteryList = Linq.new(to)
		:select(function(kv) return kv[2] end)
		:where(function(mastery) return mastery.Lv > 0 and (mastery.Category.EquipSlot == 'None' or mastery.ExtractItem == 'None') end)
		:select(function(mastery) return mastery.name end)
		:toList();
	
--	LogAndPrint('fromSet:', fromSet:getKeys());
--	LogAndPrint('toSet:', toSet:getKeys());
	
	-- 1. 추출할 특성들만 뽑음
	local extractSet = toSet:difference_copy(fromSet);
--	LogAndPrint('extractSet:', extractSet:getKeys());
	
	-- 2. 새로 장착할 특성들만 뽑음
	local equipSet = fromSet:difference_copy(toSet);
--	LogAndPrint('equipSet:', equipSet:getKeys());
	
	-- 3. 가만히 놔둘 특성들만 뽑음
	local remainSet = fromSet:intersection_copy(toSet);
--	LogAndPrint('remainSet:', remainSet:getKeys());	

	-- 4. 장착할 특성 보유 체크
	local notEnoughSet = Linq.new(equipSet)
		:select(function(kv) return kv[1] end)
		:where(function(masteryName) return company.Mastery[masteryName].Amount <= 0 end)
		:toSet();
	
	equipSet:difference(notEnoughSet);
--	LogAndPrint('equipSet2:', equipSet:getKeys());
	
	-- 5. 장착 불가 특성 체크
	local mbm = game.MasteryBoardManagerCpp:new_local();
	mbm:setPC(pcInfo);
	for _, basicMastery in ipairs(basicMasteryList) do
		mbm:addMastery(basicMastery, true);
	end
	
	local testSet = equipSet:union_copy(remainSet);
--	LogAndPrint('testSet:', testSet:getKeys());
	for masteryName, _ in pairs(testSet) do
		mbm:addMastery(masteryName, true);
	end
	
	local notEnableSet = Set.new();
	local extractMasteryFunc = function(mastery)
		if mastery.FixedMastery then
			return false;
		end	
		mbm:removeMastery(mastery.name);
		notEnableSet:insert(mastery.name);
		return true;
	end;
	local scoreFunc = function(mastery)
		return (mastery.InvalidatePC and 0 or 1000) + mastery.Cost;
	end	
	
	local categoryList = { 'Basic', 'Sub', 'Attack', 'Defence', 'Ability' };
	-- 기계는 Basic 슬롯에 다른 슬롯에 영향을 주는 특성이 많으므로 역순으로 체크
	if pcInfo.RosterType == 'Machine' then
		categoryList = { 'Ability', 'Defence', 'Attack', 'Sub', 'Basic' };
	end
	
	-- 착용 불가능한 것들 제거
	for _, key in ipairs(categoryList) do
		local masteryList = mbm:getMasteryByCategory(key);
		table.scoresort(masteryList, scoreFunc);
		for i = #masteryList, 1, -1 do
			local mastery = masteryList[i];	
			local reason = {};
			local isEnable = IsEnableByMasteryCheckType(pcInfo, mastery, reason);
			if not isEnable then
				extractMasteryFunc(mastery);
			end
		end
	end
	-- 슬롯 개수 초과한 것들 제거
	...
	-- 카테고리 코스트가 넘었으면 마지막 슬롯부터 제거 (모든 카테고리의 코스트가 만족될 때까지 반복)
	while not mbm:isValidCost() do
		for _, key in ipairs(categoryList) do
			local masteryList = mbm:getMasteryByCategory(key);
			table.scoresort(masteryList, scoreFunc);
			for i = #masteryList, 1, -1 do
				if mbm:isValidCost(key) then
					break;
				end
				local mastery = masteryList[i];
				extractMasteryFunc(mastery);
	...
	-- TP가 모자르면 0 이상이 될 때까지 하나씩 제거해봄 (특성 제거로 TP나 카테고리 코스트가 바뀔 수 있으므로 매번 검사해야 함)
	while not mbm:isValid() do
		local extracted = false;
		for _, key in ipairs(categoryList) do
			local masteryList = mbm:getMasteryByCategory(key);
			table.scoresort(masteryList, scoreFunc);
			if #masteryList > 0 then
				local mastery = masteryList[#masteryList];
				extracted = extractMasteryFunc(mastery);
				if extracted then
					break;
				end
			end
		...
		if not extracted then
			LogAndPrint('WTF? - mbm:', mbm:dumpStr());
			break;
		end
	end
--	LogAndPrint('notEnableSet:', notEnableSet:getKeys());
	
	-- 6. 장착 불가능한 특성 제외
	equipSet:difference(notEnableSet);
--	LogAndPrint('equipSet3:', equipSet:getKeys());
	
	-- 7. 가만히 놔둘 특성들 중에서 장착 불가능한 특성은 추출 대상으로
	extractSet:union(notEnableSet:intersection_copy(remainSet));
--	LogAndPrint('extractSet2:', extractSet:getKeys());		
		
	-- 2. 추출 시 필요한 재료 여부 존재. 마스터리 코스트 만큼 필요.
	local needItemCount = {};
	if not StringToBool(company.CheatState.NoUseManual) then
		local masteryList = GetClassList('Mastery');
		for masteryName, _ in pairs(extractSet) do
			local mastery = masteryList[masteryName];	
			needItemCount[mastery.ExtractItem] = (needItemCount[mastery.ExtractItem] or 0) + mastery.Cost;
		end
		for extractItem, needCount in pairs(needItemCount) do
	...
	return isEnable, reason, extractSet:getKeys(), needItemCount, equipSet:getKeys(), notEnoughSet:getKeys(), notEnableSet:getKeys();
end