...
function GetAbilityUseType(ability)
	...
	elseif ability.Type == 'Tactics'then
		abilityUseType = 'Tactics';
	end	
...
function GetEnableProtocolAbilityList(self, curAbility)
	...
	local commanderAbility = GetWithoutError(curAbility, 'CommanderAbility');
	local ret = table.filter(abilityList, function(ability)
		...
		if curAbility ~= nil and ability.Cool > 0 and (ability.name == curAbility.name or ability.name == commanderAbility) then
			if ability.Cool == ability.CoolTime + 1 then
...
function FunctionProperty_Ability_TestRequireJob(ability, jobName)
	if ability.Category == 'None' then
		return true;
	end
	local categoryCls = GetClassList('AbilityCategory')[ability.Category];
	if not categoryCls or #categoryCls.RequireJobMastery == 0 then
		return true;
	end
	local jobMastery = SafeIndex(GetClassList('Job'), jobName, 'BasicMastery');
	for _, masteryCls in pairs(categoryCls.RequireJobMastery) do
		if masteryCls.name == jobMastery then
			return true;
		end
	end
	return false;
end
---@alias AbilitySlotManager_SlotData {Active:table,Candidates:table,ActiveLimit:integer}
---- AbilitySlotManager
---@class AbilitySlotManager
---@field Basic AbilitySlotManager_SlotData
---@field Normal AbilitySlotManager_SlotData
---@field Ultimate AbilitySlotManager_SlotData
---@field OriginalData AbilitySlotManager
---@field Target roster
...
function AbilitySlotManager.new(pcInfo, presetIndex, job, autoFill)
	...
	if pcInfo.RosterType == 'Pc' then
		if presetIndex == nil then
			...
			elseif preset.Order == presetIndex and presetRedirect[presetIndex] ~= nil then
...
function AbilitySlotManager.export(self)
	return {Basic = self.Basic, Normal = self.Normal, Ultimate = self.Ultimate, OriginalData = self.OriginalData, Target = self.Target, JobName = self.JobName};
end
...
function AbilitySlotManager.Activate(self, ability)
	...
	-- 직업 특성 체크
	if not ability:TestRequireJob(self.JobName) then
		return false;
	end
	-- 이미 들어가 있음
...
function AbilitySlotManager.IsValid(self)
	for _, slotType in pairs({'Basic', 'Normal', 'Ultimate'}) do
		local slotData = self[slotType];
		-- 슬롯 개수 체크
		if slotData.ActiveLimit < #slotData.Active then
			return false;
		end
		-- 직업 체크
		if table.exist(slotData.Active, function(data)
			return not data.Ability:TestRequireJob(self.JobName);
		end) then
			return false;
		end
	end
...
function AbilitySlotManager.GetTruncatedAbilities(self)
	return table.flatten(table.map({'Basic', 'Normal', 'Ultimate'}, function (slot)
		local ret = {};
		local slotData = self[slot];
		for i, data in ipairs(slotData.Active) do
			-- 직업 테스트
			if not data.Ability:TestRequireJob(self.JobName) then
				table.insert(ret, { Ability = data.Ability.name, Reason = 'RequireJob' });
			end
			-- 슬롯 개수
			if i - #ret > slotData.ActiveLimit then
				table.insert(ret, { Ability = data.Ability.name, Reason = 'ActiveLimit' });
			end
		end
		return ret;
	end));
...
function ModifyAbilityChecker(mastery, ability, owner)
	...
	elseif mastery.ModifyAbility == 'Custom' then
		...
		if checkFunc then
			return checkFunc(mastery, ability, owner);
		end
		return false;
	else
...
function ModifyAbilityChecker_Buff(buff, ability, owner)
	...
	elseif buff.ModifyAbility == 'Custom' then
		...
		---@diagnostic disable-next-line: redundant-parameter
		if checkFunc then
			return checkFunc(buff, ability, owner);
		end
		return false;
	else
...
function GetAbilityModifyMasteryList(ability, owner, masteryTable)
	...
	-- 서브 어빌리티의 ModifyMasteryList 프로퍼티도 포함
	if ability.IsSubAbilityTooltip then
		local subAbilityList = GetAbilitySubAbilityList(ability, owner);
		for _, subAbility in ipairs(subAbilityList) do
			if subAbility.IsBuffTooltip and subAbility.ModifyMasteryList ~= '' then
				local subAppliedList = UnpackTableFromString(subAbility.ModifyMasteryList);
				for _, masteryName in ipairs(subAppliedList) do
					if not table.find(appliedList, masteryName) then
						table.insert(appliedList, masteryName);
					end
				end
			end
		end
...
function ModifyAbilityByMastery(ability, owner, masteryTable)
	...
	if ability.name == nil then
		LogAndPrint('ModifyAbilityByMastery', owner.name, 'A ability is nil');
		return;
	end
	if not masteryTable then
		masteryTable = GetMastery(owner);
	...
	for key, mastery in pairs(masteryTable) do
		if mastery.Lv > 0 and mastery.ModifyAbilityType ~= 'None' then
			table.bininsert(candidate, mastery, function(mastery) return mastery.ModifyAbilityOrder.Order; end);
		end	
	end
	...
	for _, mastery in ipairs(candidate) do
		local enable, hidden = ModifyAbilityChecker(mastery, ability, owner);
		if enable then
			local customFunc = _G['ModifyAbilityByMastery_'..mastery.ModifyAbilityType];
			if customFunc then
				---@diagnostic disable-next-line: redundant-parameter
				local associatedMastery = {customFunc(ability, owner, mastery, appliedList)};
				if not hidden then
	...
	-- 어빌리티 툴팁용 버프 갱신
	local buffKeyList = {'ApplyTargetBuff', 'ApplyTargetSubBuff', 'ApplyTargetThirdBuff'};
	for _, key in ipairs(buffKeyList) do
		local buff = ability[key];
		if buff and buff.name and buff.name ~= nil then
			local modifiedByBuff = ModifyAbilityBuffByMastery(buff, masteryTable);
			modified = modified or modifiedByBuff;
		end
	end
	
	if modified then
		AdjustAbilityModifiedStat(ability);
		InvalidateObject(ability);
	end
end
function ModifyAbilityBuffByMastery(buff, masteryTable)
	local modified = false;
	for key, mastery in pairs(masteryTable) do
		local modifier = GetWithoutError(mastery, 'BuffModifier');
		if mastery.Lv > 0 and modifier then
			local modifierForBuff = modifier[buff.name];
			if modifierForBuff then
				modifierForBuff(buff);
				modified = true;
			end
		end
	end
...
function ModifyAbilityByBuff(ability, owner)
	...
	for _, buff in pairs(GetBuffList(owner)) do
		if buff.Lv > 0 and buff.ModifyAbilityType ~= 'None' then
			table.bininsert(candidate, buff, function(buff) return buff.ModifyAbilityOrder.Order; end);
		end	
	end
	...
	for _, buff in ipairs(candidate) do
		local enable, hidden = ModifyAbilityChecker_Buff(buff, ability, owner);
		if enable then
...
function AdjustAbilityModifiedStat(ability)
	if IsClass(ability) then
		return;
	end
	local host = GetHostClass(ability);
	for _, stat in ipairs({ 'CastDelay', 'Cost' }) do
		local hostStat = GetWithoutError(host, stat);
		local abilityStat = GetWithoutError(ability, stat);
		if abilityStat ~= nil and abilityStat ~= hostStat then
			local reducedStat = hostStat - abilityStat;
			ability[stat] = hostStat - math.floor(reducedStat + 0.001);
		end
	end
end
---------------------------------------------------------------
...
function ModifyAbilityByMastery_TargetRange(ability, owner, mastery)
	ability.TargetRange = GetExpandRangeAmount(ability.TargetRange, mastery.ApplyAmount);
end
...
function ModifyAbilityByMastery_CastDelay(ability, owner, mastery)
	...
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * mastery.ApplyAmount / 100));
end
-- 재정비 / 쿨 다운 - CoolTime
function ModifyAbilityByMastery_CoolTime_Common(ability, owner, mastery, propName)
	local applyAmount = GetWithoutError(mastery, propName) or 0;
	if ability.CoolTime > 0 and applyAmount > 0 then
		ability.CoolTime = math.max(0, ability.CoolTime - applyAmount);
	end
end
function ModifyAbilityByMastery_CoolTime(ability, owner, mastery)
	ModifyAbilityByMastery_CoolTime_Common(ability, owner, mastery, 'ApplyAmount');
end
function ModifyAbilityByMastery_CoolTime2(ability, owner, mastery)
	ModifyAbilityByMastery_CoolTime_Common(ability, owner, mastery, 'ApplyAmount2');
end
-- 융화 / 코스트 - Cost
function ModifyAbilityByMastery_Cost(ability, owner, mastery)
	ability.Cost = ( ability.Cost * mastery.ApplyAmount / 100 );
end
---------------------------------------------------------------
-- 턴 플레이 타입
---------------------------------------------------------------
...
---------------------------------------------------------------
-- 개별 마스터리 적용.
-----------------------------------------*MASTERY_ABILITY_MODIFIERS
-- 생체 전기
---@param mastery class_Mastery
---@param ability class_Ability
function ModifyAbilityChecker_Bioelectricity(mastery, ability)
	return IsGetAbilitySubType(ability, mastery.Type.name) and ability.Cost > 0
end
---@param ability class_Ability
---@param owner unit
---@param mastery class_Mastery
function ModifyAbilityByMastery_Bioelectricity(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	local reduceAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2;
	ability.Cost = math.max(0, ability.Cost - (host.Cost * reduceAmount / 100));
end

-- 실바람
---@param mastery class_Mastery
---@param ability class_Ability
function ModifyAbilityChecker_LightBreeze(mastery, ability)
	return IsGetAbilitySubType(ability, mastery.Type.name) and ability.CastDelay > 0;
end
---@param ability class_Ability
---@param owner unit
---@param mastery class_Mastery
function ModifyAbilityByMastery_LightBreeze(ability, owner, mastery)
	local host = GetHostClass(ability);
	local applyAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2;
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * applyAmount / 100));
end

-- 끊임없는 파도
---@param mastery class_Mastery
---@param ability class_Ability
function ModifyAbilityChecker_Surgingwaves(mastery, ability)
	return ability.Type == 'Attack' and IsGetAbilitySubType(ability, 'Water');
end
---@param ability class_Ability
---@param owner unit
---@param mastery class_Mastery
function ModifyAbilityByMastery_Surgingwaves(ability, owner, mastery)
	local addAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2;
	if GetWithoutError(ability.AdditionalApplyAmount, 'ESPPower') then
		ability.AdditionalApplyAmount.ESPPower = ability.AdditionalApplyAmount.ESPPower + addAmount;
	end
end
...
function ModifyAbilityChecker_HappySong(mastery, ability)
	return ability.Category == 'Song';
end
function ModifyAbilityByMastery_HappySong(ability, owner, mastery)
	local additiveEffect = {};
	local masteryTable = GetMastery(owner);
	-- 내 노래를 들어봐!
	local mastery_ListenToMySong = GetMasteryMastered(masteryTable, 'ListenToMySong');
	if mastery_ListenToMySong then
		ModifyAbilityByMastery_CoolTime_Common(ability, owner, mastery_ListenToMySong, 'ApplyAmount');
		table.insert(additiveEffect, mastery_ListenToMySong);
	end
	-- 힘내세요!
	local mastery_SongForYou = GetMasteryMastered(masteryTable, 'SongForYou');
	if mastery_SongForYou and (ability.Type == 'Heal' or ability.Type == 'Assist') and ability.TurnPlayType ~= 'Terminate' then
		ability.TurnPlayType = 'Free';
		table.insert(additiveEffect, mastery_SongForYou);
	else
		if ability.TurnPlayType == 'Main' then
			ability.TurnPlayType = 'Half';
		elseif ability.TurnPlayType == 'Half' then
...
function ModifyAbilityChecker_Training100(mastery, ability)
	-- 타박상 확률 증가 (ApplyTargetBuff가 없거나, 이미 타박상이거나)
	if ability.name == 'FlameKick' then
		return ability.ApplyTargetBuff.name == nil or ability.ApplyTargetBuff.name == mastery.Buff.name;
	end
	return Set.new({'FlameShoot', 'FlameDance', 'FlameCarriage', 'FlameJumpKick', 'FlameBackJumpKick', 'FlameStampKick', 'ResentmentOfRedLesserPanda', 'LesserDeepBreath'})[ability.name];
end
function ModifyAbilityByMastery_Training100(ability, owner, mastery)
	...
	elseif ability.name == 'FlameKick'then
		-- ApplyTargetBuff가 없으면 추가
		if ability.ApplyTargetBuff.name == nil then
			ability.ApplyTargetBuff = GetClassList('Buff')[mastery.Buff.name];
		end
	...
	elseif ability.name == 'FlameCarriage'then
		ability.DistanceAttackApplyAmountRatio = ability.DistanceAttackApplyAmountRatio + mastery.ApplyAmount3;
	...
	elseif ability.name == 'FlameStampKick'then
		ability.Cost = ability.Cost - (ability.Cost * mastery.ApplyAmount2 / 100);
...
function ModifyAbilityChecker_Training100_Leton(mastery, ability)
	-- 타박상 확률 증가 (ApplyTargetBuff가 없거나, 이미 타박상이거나)
	if ability.name == 'BodyBlow' then
		return ability.ApplyTargetBuff.name == nil or ability.ApplyTargetBuff.name == mastery.Buff.name;
	end
	return Set.new({'WarmUp', 'Tumbling', 'Provocation', 'IceArrow', 'IceSpear', 'Twister', 'FrostSpinkick', 'FrostDropkick', 'FrostFinalKick', 'Frostmill'})[ability.name];
end
...
function ModifyAbilityByMastery_DivideAndConquerAlgorithm(ability, owner, mastery)
	...
	elseif Set.new({'RestoreCommand', 'AwakenCommand'})[ability.name] then
...
function ModifyAbilityByMastery_Avalon(ability, owner, mastery)
	...
	ability.Cost = math.max(0, ability.Cost - (host.Cost * reduceRatio / 100));
end
-- 저전력
...
function ModifyAbilityByMastery_MachineUnique_LowEnergy(ability, owner, mastery)
	...
	ability.Cost = math.max(0, ability.Cost - (host.Cost * mastery.ApplyAmount / 100));
end
-- 호환성 증가 - 시전 지연 시간
...
function ModifyAbilityChecker_AcuteClaw(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.ApplyTargetBuff.name == mastery.SubBuff.name and ability.ApplyTargetBuffLv > 0;
end
...
function ModifyAbilityChecker_StrongRush(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Blunt' and ability.ApplyTargetBuff.name == mastery.SubBuff.name and ability.ApplyTargetBuffLv > 0;
end
function ModifyAbilityByMastery_StrongRush(ability, owner, mastery)
	ability.ApplyTargetBuff = GetClassList('Buff')[mastery.Buff.name];
	local additionalMasteries = {};
	return unpack(additionalMasteries);
end
-- 거침없는 야수
function ModifyAbilityChecker_GreatHeavyStrike(mastery, ability)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	-- StrongRush/HeavyBody 판정은 각 특성의 실제 버프 컬럼(SubBuff 등) 기준이어야 한다.
	-- 기존엔 GreatHeavyStrike(SubBuff=None)를 그대로 넘겨 StrongRush 가지가 항상 실패 → HeavyBody 가지의
	-- ApplyTargetBuffChance<100 조건에만 의존, 100%확률 과충전 어빌(RollingAttack2 등)이 탈락했다. (#16518)
	local strongRush = GetClassList('Mastery')['StrongRush'];
	local heavyBody = GetClassList('Mastery')['HeavyBody'];
	return ModifyAbilityChecker_StrongRush(strongRush, host) or ModifyAbilityChecker_StrongRush(strongRush, ability)
		or ModifyAbilityChecker_HeavyBody(heavyBody, ability);
end
function ModifyAbilityByMastery_GreatHeavyStrike(ability, owner, mastery, candidates)
	for _, candidate in ipairs(candidates) do
		if candidate.name == 'StrongRush' then
			ability.AdditionalApplyAmount.AttackPower = ability.AdditionalApplyAmount.AttackPower + math.floor((#candidates + 1) / mastery.ApplyAmount) * mastery.ApplyAmount2;
		elseif candidate.name == 'HeavyBody' then
			ability.AdditionalApplyAmount.AttackPower = ability.AdditionalApplyAmount.AttackPower + math.floor((#candidates + 1) / mastery.ApplyAmount3) * mastery.ApplyAmount4;
		end
	end
end
-- 찍어 누르기
function ModifyAbilityChecker_PushDown(mastery, ability)
	-- 낙하 공격
	return ability.HitRateType == 'Fall' and ability.Type == 'Attack';
end
function ModifyAbilityByMastery_PushDown(ability, owner, mastery)
	...
	-- 야샤 투하
	local mastery_YashaDrop = GetMasteryMastered(GetMastery(owner), 'YashaDrop');
	if mastery_YashaDrop then
		addAmount = addAmount + mastery_YashaDrop.ApplyAmount;
		table.insert(additionalMasteries, mastery_YashaDrop);
	end
	ability.NoCoverAttackApplyAmountRatio = ability.NoCoverAttackApplyAmountRatio + addAmount;
	return unpack(additionalMasteries);
end
-- 붉은 가시 검
...
function ModifyAbilityChecker_Bloodlet(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.ApplyTargetBuff.Group == mastery.BuffGroup.name;
end
...
function ModifyAbilityChecker_Bruise(mastery, ability)
	return ability.Type == 'Attack' and ability.HitRateType == 'Melee' and (ability.ApplyTargetBuff.name == nil or ability.ApplyTargetBuff.Group == mastery.BuffGroup.name);
end
...
function ModifyAbilityChecker_IronFist(mastery, ability)
	return ability.Type == 'Attack' and ability.HitRateType == 'Melee' and (ability.ApplyTargetBuff.name == mastery.SubBuff.name or ability.ApplyTargetBuff.name == mastery.Buff.name) and ability.ApplyTargetBuffLv > 0;
end
function ModifyAbilityByMastery_IronFist(ability, owner, mastery)
	if ability.ApplyTargetBuff.name ~= mastery.Buff.name then
		ability.ApplyTargetBuff = GetClassList('Buff')[mastery.Buff.name];
	end
	ability.ApplyTargetBuffChance = math.min(ability.ApplyTargetBuffChance + mastery.ApplyAmount, 100);
end
...
function ModifyAbilityChecker_ElectricShock(mastery, ability)
	return ability.Type == 'Attack' and ability.ApplyTargetBuff.Group == mastery.BuffGroup.name and ability.ApplyTargetBuffLv > 0;
end
...
function ModifyAbilityByMastery_RainShower(ability, owner, mastery)
	...
	-- 대홍수
	local mastery_Deluge = GetMasteryMastered(GetMastery(owner), 'Deluge');
	if mastery_Deluge then
		applyAmount = applyAmount + mastery_Deluge.ApplyAmount;
		table.insert(additiveEffect, mastery_Deluge);
	end
...
-- 마법 개량
function ModifyAbilityChecker_MagicImprovement(mastery, ability)
	return IsGetAbilitySubType(ability, 'ESP') and ability.CastDelay > 0;
end
function ModifyAbilityByMastery_MagicImprovement(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
function ModifyAbilityByMastery_ArchWitch(ability, owner, mastery)
	...
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * mastery.ApplyAmount2 / 100));
end
-- 날카로운 이빨
...
function ModifyAbilityByMastery_GunbarrelEnhancement(ability, owner, mastery)
	...
	if ability.Target == 'Self' or ability.Target == 'Ground' or ability.Target == 'EmptyGround' then
		ability.ApplyRange = GetExpandRangeAmount(ability.ApplyRange, mastery.ApplyAmount);
		ability.GuideRange = GetExpandRangeAmount(ability.GuideRange, mastery.ApplyAmount);
	end
end
-- 빨리 던지기.
...
function ModifyAbilityByMastery_Module_Overclocking(ability, owner, mastery)
	...
	ability.Cost = ability.Cost + (host.Cost * mastery.ApplyAmount / 100);
	local applyAmount2 = mastery.ApplyAmount2;
	if owner.ESP and owner.ESP.name == 'Heat' then
		applyAmount2 = applyAmount2 + mastery.ApplyAmount3;
	end
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * applyAmount2 / 100));
end
-- 관통 사격
...
function ModifyAbilityChecker_Drone_MonoAndVivi_V6(mastery, ability, owner)
	return ability.Type == 'Attack' and IsProtocolAbility(ability);
end
function ModifyAbilityChecker_Drone_MonoAndVivi_V7(mastery, ability, owner)
	return ability.Type == 'Attack' and IsProtocolAbility(ability);
end
function ModifyAbilityChecker_Drone_MonoAndVivi_V8(mastery, ability, owner)
	return ability.Type == 'Attack' and IsProtocolAbility(ability);
end
...
function ModifyAbilityChecker_Drone_MonoAndVivi_M6(mastery, ability, owner)
	return ability.Type == 'Assist' and IsProtocolAbility(ability);
end
function ModifyAbilityChecker_Drone_MonoAndVivi_M7(mastery, ability, owner)
	return ability.Type == 'Assist' and IsProtocolAbility(ability);
end
function ModifyAbilityChecker_Drone_MonoAndVivi_M8(mastery, ability, owner)
	return ability.Type == 'Assist' and IsProtocolAbility(ability);
end
function ModifyAbilityChecker_Drone_Mechanic_Set(mastery, ability, owner)
	return IsProtocolAbility(ability);
end
-- 기계장인 지원 키트
function ModifyAbilityChecker_Amulet_Mechanic_Set(mastery, ability, owner)
	return ability.Type == 'Assist' and IsProtocolAbility(ability);
end
-- 흑철 가죽 구두
function ModifyAbilityChecker_DressShoes_BlackIronEnhanced(mastery, ability, owner)
...
-- 멍고 모피 부츠 시리즈
function ModifyAbilityChecker_Boots_Munggo(mastery, ability, owner)
	return ability.Type == 'Attack' and ability.CastDelay > 0;
end
function ModifyAbilityChecker_Boots_Munggo_Rare(mastery, ability, owner)
	return ModifyAbilityChecker_Boots_Munggo(mastery, ability, owner);
end
function ModifyAbilityChecker_Boots_Munggo_Epic(mastery, ability, owner)
	return ModifyAbilityChecker_Boots_Munggo(mastery, ability, owner);
end
-- 흑철 가죽 코트
...
function ModifyAbilityChecker_WindWitch(mastery, ability, owner)
	return Set.new({'WindArmor', 'FakeEye', 'WindwhirlBullet', 'WindKick', 'WhirlWind', 'WindWhip', 'Windwalker', 'WindChain', 'WindwhirlArrow', 'WindwhirlDance', 'WindBurst'})[ability.name];
end
function ModifyAbilityByMastery_WindWitch(ability, owner, mastery)
	if ability.name == 'WindArmor' or ability.name == 'FakeEye' then
		if ability.TurnPlayType == 'Main' then
			ability.TurnPlayType = 'Half';
		elseif ability.TurnPlayType == 'Half' then
			ability.TurnPlayType = 'Free';
		end
	elseif ability.name == 'WindwhirlBullet' or ability.name == 'WindKick' or ability.name == 'WhirlWind' or ability.name == 'WindWhip' then
	...
	elseif ability.name == 'Windwalker' then
		ability.TargetRange = GetExpandRange(ability.TargetRange);
	elseif ability.name == 'WindChain' then
		ability.MaxUseCount = ability.MaxUseCount + mastery.ApplyAmount3;
	elseif ability.name == 'WindwhirlArrow' then
		ability.DistanceAttackApplyAmountRatio = ability.DistanceAttackApplyAmountRatio + mastery.ApplyAmount4;
	elseif ability.name == 'WindwhirlDance' then
		ability.NotMoveAttackApplyAmountRatio = ability.NotMoveAttackApplyAmountRatio + mastery.ApplyAmount5;
	elseif ability.name == 'WindBurst' then		
		ability.NoCoverAttackApplyAmountRatio = ability.NoCoverAttackApplyAmountRatio + mastery.ApplyAmount5;
	end	
end
-- 백열
...
function ModifyAbilityByMastery_TrapHunter(ability, owner, mastery)
	...
	ability.ApplyTargetThirdBuff = GetClassList('Buff')[mastery.Buff.name];
	ability.ApplyTargetThirdBuffChance = mastery.ApplyAmount2;
	ability.ApplyTargetThirdBuffLv = 1;
end
-- 코드 최적화.
...
function ModifyAbilityByMastery_Algorithm(ability, owner, mastery)
	...
	local removeDelay = (host.CastDelay * mastery.ApplyAmount2 * math.floor(mastery.CustomCacheData / mastery.ApplyAmount) / 100);
...
function ModifyAbilityByMastery_EnhancedConveterAI(ability, owner, mastery)
	...
	local removeCost = (host.Cost * mastery.ApplyAmount / 100);
...
function ModifyAbilityByMastery_Windows(ability, owner, mastery)
	...
	local addCost = (host.Cost * mastery.ApplyAmount2 * math.floor(mastery.CustomCacheData / mastery.ApplyAmount) / 100);
	ability.Cost = ability.Cost + addCost;
	local removeDelay = (host.CastDelay * mastery.ApplyAmount3 * math.floor(mastery.CustomCacheData / mastery.ApplyAmount) / 100);
...
function ModifyAbilityByMastery_Linux(ability, owner, mastery)
	...
	local addCost = -1 * (host.Cost * mastery.ApplyAmount2 * math.floor(mastery.CustomCacheData / mastery.ApplyAmount) / 100);
...
-- 조준 보조 연산 장치
function ModifyAbilityChecker_Module_AimCalculator(mastery, ability)
	return ability.Type == 'Attack';
end
function ModifyAbilityByMastery_Module_AimCalculator(ability, owner, mastery)
	ability.Accuracy = ability.Accuracy + mastery.ApplyAmount2;
	ability.CriticalStrikeChance = ability.CriticalStrikeChance + mastery.ApplyAmount2;
	local host = ability;
	if not IsClass(ability) then
...
function ModifyAbilityChecker_AttackSupportDevice_AimAssist_Legend(mastery, ability, owner)
	return SafeIndex(owner, 'Weapon', 'Type', 'name') == 'OuterDevice_FlameThrower' and ability.name == mastery.Ability;
end
function ModifyAbilityByMastery_AttackSupportDevice_AimAssist_Legend(ability, owner, mastery)
	ability.Active = false;
end
-- 리펙토링
...
function ModifyAbilityByMastery_CodeRefactoring(ability, owner, mastery)
	...
	ability.Cost = math.max(0, ability.Cost - (host.Cost * mastery.ApplyAmount2 / 100));
end
-- 다중 연산장치
...
function ModifyAbilityByMastery_Module_GoodEnergy(ability, owner, mastery)
	...
	local applyAmount = mastery.ApplyAmount;
	if owner.Info.name == 'Drone_Transport' then
		applyAmount = applyAmount + mastery.ApplyAmount2;
	end
	-- 연비 강화 프로그램
	...
	local addCost = -1 * (host.Cost * applyAmount / 100);
	ability.Cost = math.max(0, ability.Cost + addCost);
	return unpack(additiveEffect);
end
-- 구동 호환성 - 고속
...
function ModifyAbilityChecker_DrivingDevice_HoverSpeed_Legend(mastery, ability, owner)
	return owner.Info.name == 'Drone_Speed' and ability.Cost > 0;
end
function ModifyAbilityByMastery_DrivingDevice_HoverSpeed(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	local addCost = -1 * (host.Cost * mastery.ApplyAmount / 100);
	ability.Cost = math.max(0, ability.Cost + addCost);
end
-- 연료 호환성 - 수송
...
-- 강철 장벽
function ModifyAbilityChecker_IronWall(mastery, ability)
	return ability.name == 'IronWall', true;
end
function ModifyAbilityByMastery_IronWall(ability, owner, mastery)
	-- 서리 요새
	local mastery_FortressOfFrost = GetMasteryMastered(GetMastery(owner), 'FortressOfFrost');
...
-- 전사의 후예
function ModifyAbilityChecker_DescendantOfWarrior(mastery, ability)
	return ability.name == 'Tima_AttackMode' or ability.name == 'Tima_ChaserMode', true;
end
function ModifyAbilityByMastery_DescendantOfWarrior(ability, owner, mastery)
	-- 전사의 계승자
...
-- 수호자의 후예
function ModifyAbilityChecker_DescendantOfGuardian(mastery, ability)
	return ability.name == 'Tima_DefenceMode' or ability.name == 'Tima_CheckMode', true;
end
function ModifyAbilityByMastery_DescendantOfGuardian(ability, owner, mastery)
	-- 수호자의 계승자
...
function ModifyAbilityChecker_Bangle_JewelCollectorExpert_Set(mastery, ability)
	return ModifyAbilityChecker_ESPAccelerator(mastery, ability);
end
function ModifyAbilityChecker_Bangle_JewelCollectorMaster_Set(mastery, ability)
	return ModifyAbilityChecker_ESPAccelerator(mastery, ability);
end
function ModifyAbilityChecker_Bangle_JewelCollectorGrandMaster_Set(mastery, ability)
	return ModifyAbilityChecker_ESPAccelerator(mastery, ability);
end
--- 유니콘 단검
function ModifyAbilityChecker_Dagger_Unicorn(mastery, ability)
	return IsGetAbilitySubType(ability, 'ESP') and ability.CastDelay > 0;
end
function ModifyAbilityChecker_Dagger_Unicorn_Rare(mastery, ability)
	return ModifyAbilityChecker_Dagger_Unicorn(mastery, ability);
end
function ModifyAbilityChecker_Dagger_Unicorn_Epic(mastery, ability)
	return ModifyAbilityChecker_Dagger_Unicorn(mastery, ability);
end
...
function ModifyAbilityChecker_Jacket_ExtractorExpert_Set(mastery, ability)
	return ModifyAbilityChecker_ExtractorUseCount(mastery, ability);
end
function ModifyAbilityChecker_Jacket_ExtractorMaster_Set(mastery, ability)
	return ModifyAbilityChecker_ExtractorUseCount(mastery, ability);
end
function ModifyAbilityChecker_Jacket_ExtractorGrandMaster_Set(mastery, ability)
	return ModifyAbilityChecker_ExtractorUseCount(mastery, ability);
end
-- 추출업자 코트
...
function ModifyAbilityChecker_Coat_ExtractorExpert_Set(mastery, ability)
	return ModifyAbilityChecker_ExtractorUseCount(mastery, ability);
end
function ModifyAbilityChecker_Coat_ExtractorMaster_Set(mastery, ability)
	return ModifyAbilityChecker_ExtractorUseCount(mastery, ability);
end
function ModifyAbilityChecker_Coat_ExtractorGrandMaster_Set(mastery, ability)
	return ModifyAbilityChecker_ExtractorUseCount(mastery, ability);
end
...
function ModifyAbilityByMastery_BoilingPoison(ability, owner, mastery)
	...
	if mastery_Poisonmage then
		applyAmount = applyAmount + mastery_Poisonmage.ApplyAmount4;
		table.insert(additiveEffect, mastery_Poisonmage);
	end
	ability.ApplyTargetBuffChance = math.min(ability.ApplyTargetBuffChance + applyAmount, 100);
	return unpack(additiveEffect);
end
...
function ModifyAbilityByMastery_LongTimeinfection(ability, owner, mastery)
	...
	MultiColumnModifier(ability, host, 'ApplyAmountChangeStep', function(v, vHost) return v + math.floor(vHost * math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2 / 100) end);
end
-- 번지는 불길
...
function ModifyAbilityChecker_GoodChopping(mastery, ability)
	return ability.Type == 'Attack' and ability.HitRateType == 'Melee' and IsGetAbilitySubType(ability, 'Slashing');
end
...
-- 괴력 집게
function ModifyAbilityChecker_PowerfulCrabClaws(mastery, ability)
	return ability.Type == 'Attack' and ability.HitRateType == 'Melee';
end
function ModifyAbilityByMastery_PowerfulCrabClaws(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
function ModifyAbilityByMastery_GunCompatibility(ability, owner, mastery)
	local additiveEffect = {};
	-- 고속 철갑탄
	local mastery_HighVelocityIronBullet = GetMasteryMastered(GetMastery(owner), 'HighVelocityIronBullet');
	if mastery_HighVelocityIronBullet then
		table.insert(additiveEffect, mastery_HighVelocityIronBullet);
	end
	return unpack(additiveEffect);
end
function ModifyAbilityFinalizerByMastery_GunCompatibility(ability, owner, mastery, appliedMasteries)
	if not GetWithoutError(ability.AdditionalApplyAmount, 'AttackPower') then
		return;
	end
	local addValue = math.floor(#appliedMasteries / mastery.ApplyAmount) * mastery.ApplyAmount2;
	-- 고속 철갑탄
	local mastery_HighVelocityIronBullet = GetMasteryMastered(GetMastery(owner), 'HighVelocityIronBullet');
	if mastery_HighVelocityIronBullet then
		addValue = addValue + math.floor(#appliedMasteries / mastery_HighVelocityIronBullet.ApplyAmount3) * mastery_HighVelocityIronBullet.ApplyAmount4;
	end
	ability.AdditionalApplyAmount.AttackPower = ability.AdditionalApplyAmount.AttackPower + addValue;
end
-- 단련된 공격
...
function ModifyAbilityByMastery_DeftnessAttack(ability, owner, mastery)
	...
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * cnt * mastery.ApplyAmount / 100));
end
-- 능숙한 손놀림
...
function ModifyAbilityChecker_ResonantCrying(mastery, ability)
	return ability.Category == 'Song' and ability.TargetType == 'Area' and (tonumber(ability.RangeRadius) or 0) > 0;
end
...
function ModifyAbilityChecker_EndlessSong(mastery, ability)
	return ability.Category == 'Song' and ability.CastDelay > 0;
end
...
function ModifyAbilityByMastery_VeteranSpirit(ability, owner, mastery)
	...
	local mastery_RealJobSpirit = GetMasteryMastered(GetMastery(owner), 'RealJobSpirit');
	if mastery_RealJobSpirit then
		applyAmount = applyAmount + math.floor(mastery_RealJobSpirit.CustomCacheData / mastery_RealJobSpirit.ApplyAmount3) * mastery_RealJobSpirit.ApplyAmount4;
		table.insert(additionalMasteries, mastery_RealJobSpirit);
	end
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * applyAmount / 100));
	return unpack(additionalMasteries);
end
...
-- 명사수의 눈
function ModifyAbilityChecker_BestSniperEye(mastery, ability)
	return ability.name == 'DistanceSnipeMode' or ability.name == 'AccuracySnipeMode';
end
function ModifyAbilityByMastery_BestSniperEye(ability, owner, mastery)
	if ability.name == 'AccuracySnipeMode' then
		ability.ApplyTargetBuff = GetClassList('Buff')[mastery.ForthBuff.name];
		ability.RemoveBuff = GetClassList('Buff')[mastery.SubBuff.name];
	else
		ability.ApplyTargetBuff = GetClassList('Buff')[mastery.SubBuff.name];
		ability.RemoveBuff = GetClassList('Buff')[mastery.ForthBuff.name];
	end
end
-- 능숙한 사냥
function ModifyAbilityChecker_SkilledHunting(mastery, ability)
	return ability.name == 'ChaserMode' or ability.name == 'StakeMode';
end
function ModifyAbilityByMastery_SkilledHunting(ability, owner, mastery)
	if ability.name == 'ChaserMode' then
		ability.ApplyTargetBuff = GetClassList('Buff')[mastery.ForthBuff.name];
		ability.RemoveBuff = GetClassList('Buff')[mastery.SubBuff.name];
	else
		ability.ApplyTargetBuff = GetClassList('Buff')[mastery.SubBuff.name];
...
-- 고무된 전사
function ModifyAbilityChecker_EncouragedWarrior(mastery, ability)
	local abilityCls = GetClassList('Ability')[ability.name];
	return ability.Type == 'Assist' and (abilityCls.TurnPlayType == 'Main' or abilityCls.TurnPlayType == 'Half');
end
-- 광전사의 분노
function ModifyAbilityChecker_MadnessRage(mastery, ability, owner)
	return ability.name == 'Frenzy2' and owner.CostType.name == 'Rage';
end
function ModifyAbilityByMastery_MadnessRage(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	ability.RestoreMaxCost = true;
	ability.Cost = math.max(0, ability.Cost - (host.Cost * mastery.ApplyAmount2 / 100));
end
-- 분노의 외침
function ModifyAbilityChecker_BarbarianShout(mastery, ability, owner)
	return ability.name == 'Encourage' and owner.CostType.name == 'Rage';
end
function ModifyAbilityByMastery_BarbarianShout(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	ability.Cost = math.max(0, ability.Cost - (host.Cost * mastery.ApplyAmount3 / 100));
	ability.ApplyTargetBuff = GetClassList('Buff')[mastery.SubBuff.name];
end
-- 대마법사의 지팡이
function ModifyAbilityChecker_ArchmageMagicWeapon(mastery, ability, owner)
	return ability.Type == 'Attack' and IsGetAbilitySubType(ability, 'ESP') and ability.CastDelay > 0;
end
-- 오만의 마녀
function ModifyAbilityChecker_WitchOfPride(mastery, ability, owner)
	return ability.name == 'BeholderVision' or ability.name == 'BeholderVision2';
end
-- 마녀의 세 번째 눈
function ModifyAbilityChecker_BeholderVision(mastery, ability)
	return ability.name == 'BeholderVision', true;
end
function ModifyAbilityByMastery_BeholderVision(ability, owner, mastery)
	-- 색욕의 마녀
	local mastery_WitchOfLust = GetMasteryMastered(GetMastery(owner), 'WitchOfLust');
	if mastery_WitchOfLust then
		ability.Active = false;
	else
		ability.Active = true;
	end
...
-- 에트로스 전투검
function ModifyAbilityChecker_Sword_Etros(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.CastDelay > 0;
end
function ModifyAbilityChecker_Sword_Etros_Rare(mastery, ability)
	return ModifyAbilityChecker_Sword_Etros(mastery, ability);
end
function ModifyAbilityChecker_Sword_Etros_Epic(mastery, ability)
	return ModifyAbilityChecker_Sword_Etros(mastery, ability);
end
--- 에트로스 전투단검
function ModifyAbilityChecker_Dagger_Etros(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.CastDelay > 0;
end
function ModifyAbilityChecker_Dagger_Etros_Rare(mastery, ability)
	return ModifyAbilityChecker_Dagger_Etros(mastery, ability);
end
function ModifyAbilityChecker_Dagger_Etros_Epic(mastery, ability)
	return ModifyAbilityChecker_Dagger_Etros(mastery, ability);
end
-- 에트로스 전투 대검
function ModifyAbilityChecker_TwoHandSword_Etros(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.CastDelay > 0;
end
function ModifyAbilityChecker_TwoHandSword_Etros_Rare(mastery, ability)
	return ModifyAbilityChecker_Sword_Etros(mastery, ability);
end
function ModifyAbilityChecker_TwoHandSword_Etros_Epic(mastery, ability)
	return ModifyAbilityChecker_Sword_Etros(mastery, ability);
end
-- 골방의 마녀
function ModifyAbilityChecker_WitchInTheRoom(mastery, ability)
	return ability.Type == 'Assist' and IsGetAbilitySubType(ability, 'ESP') and (ability.TurnPlayType ~= 'Free' or ability.CoolTime > 0);
end
function ModifyAbilityByMastery_WitchInTheRoom(ability, owner, mastery)
	if ability.TurnPlayType == 'Main' then
		ability.TurnPlayType = 'Half';
	elseif ability.TurnPlayType == 'Half' then
		ability.TurnPlayType = 'Free';
...
-- 영광의 상처
function ModifyAbilityChecker_GloriousScar(mastery, ability)
	return ability.Type == 'Attack' and ability.HitRateType == 'Melee';
end
function ModifyAbilityByMastery_GloriousScar(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 전투 태세
function ModifyAbilityChecker_StanceChange(mastery, ability)
	return ability.name == 'AttackStance' or ability.name == 'DefenceStance', true;
end
function ModifyAbilityByMastery_StanceChange(ability, owner, mastery)
	-- 능숙한 전투 태세
...
-- 개량된 주포
function ModifyAbilityChecker_EnhancedStrongCannon(mastery, ability)
	return ability.Type == 'Attack' and IsProtocolAbility(ability);
end
function ModifyAbilityByMastery_EnhancedStrongCannon(ability, owner, mastery)
	ability.TargetRange = GetExpandRange(ability.TargetRange);
	-- 적용 범위 증가
	local applyRangeCls = GetClassList('Range')[ability.ApplyRange];
	if ability.Target == 'Self' or applyRangeCls.Type == 'Fan' or applyRangeCls.Type == 'StraightLine' then
...
-- 절대 극의
function ModifyAbilityByMastery_UltraInstinct(ability, owner, mastery)
	ability.ApplyTargetBuff = GetClassList('Buff')[mastery.SubBuff.name];
end
-- 땅굴지기
function ModifyAbilityChecker_BurrowBeast(mastery, ability)
	return Set.new({'Digging', 'Digging_Disable', 'BurrowAmbush', 'Burrowwalker'})[ability.name];
end
-- 무장연금 손목 보호대
function ModifyAbilityChecker_Wrist_BattleAlchemist_Set(mastery, ability, owner)
	return ability.HitRateType == 'Throw' and #ability.ApplyFieldEffects > 0;
end
function ModifyAbilityByMastery_Wrist_BattleAlchemist_Set(ability, owner, mastery)
	for _, info in ipairs(ability.ApplyFieldEffects) do
		info.Range = GetExpandRange(info.Range);
	end
end
-- 괴수 사냥꾼 덫
function ModifyAbilityChecker_Amulet_MonsterHunter_Set(mastery, ability, owner)
	return ability.Type == 'Trap';
end
function ModifyAbilityByMastery_Amulet_MonsterHunter_Set(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 무장연금 - 2 세트
function ModifyAbilityChecker_BattleAlchemist2(mastery, ability, owner)
	return ability.ItemAbility and ability.IsUseCount;
end
-- 어둠추적자 허리띠
function ModifyAbilityChecker_Amulet_DarkChaser_Set(mastery, ability, owner)
	return ability.ItemAbility and ability.IsUseCount;
end
-- 괴수 사냥꾼 - 2 세트
function ModifyAbilityChecker_MonsterHunterSet2(mastery, ability, owner)
	return ability.Type == 'Trap';
end
function ModifyAbilityByMastery_MonsterHunterSet2(ability, owner, mastery)
	ability.GuideRange = GetExpandRange(ability.GuideRange);
	ability.PickingRange = GetExpandRange(ability.PickingRange);
end
-- 고성능 센서
function ModifyAbilityChecker_Module_EnhancedSensor(mastery, ability)
	return ability.name == 'SearchField';
end
function ModifyAbilityByMastery_Module_EnhancedSensor(ability, owner, mastery)
	ability.CancelTargetBuffLv = 1;
	ability.CancelTargetBuff = GetClassList('Buff')[mastery.Buff.name];
end
-- 바람 달리기
function ModifyAbilityByMastery_Windrunner(ability, owner, mastery)
	ability.TargetRange = GetExpandRange(ability.TargetRange);
	ability.CoolTime = math.max(0, ability.CoolTime - mastery.ApplyAmount2);
end
-- 예열 장치
function ModifyAbilityChecker_Module_PreheatDevice(mastery, ability, owner)
	return ability.CastDelay > 0;
end
function ModifyAbilityByMastery_Module_PreheatDevice(ability, owner, mastery)
	local additiveEffect = {};
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	...
	local applyAmount = mastery.ApplyAmount;
	-- 엔진 열효율 강화
	local mastery_Module_HeatCirculationEngine = GetMasteryMastered(GetMastery(owner), 'Module_HeatCirculationEngine');
	if mastery_Module_HeatCirculationEngine then
		applyAmount = applyAmount + mastery_Module_HeatCirculationEngine.ApplyAmount;
		table.insert(additiveEffect, mastery_Module_HeatCirculationEngine);
	end
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * applyAmount / 100));
	return unpack(additiveEffect);
end
-- 보조 배터리
function ModifyAbilityChecker_Module_SubBattery(mastery, ability, owner)
	return ability.Cost > 0;
end
function ModifyAbilityByMastery_Module_SubBattery(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 추가 에너지 팩
function ModifyAbilityChecker_Module_AdditionalEnergyPack(mastery, ability, owner)
	return ability.name == 'EmergencyFuel' or ability.name == 'Charging_Machine';
end
function ModifyAbilityByMastery_Module_AdditionalEnergyPack(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	if ability.name == 'EmergencyFuel' then
		ability.MaxUseCount = ability.MaxUseCount + mastery.ApplyAmount2;
	else
		ability.MaxUseCount = ability.MaxUseCount + mastery.ApplyAmount;
	end
...
-- 초고압 노즐
function ModifyAbilityChecker_Module_HighPressureNozzle(mastery, ability, owner)
	return ability.Type == 'Attack' and ability.RangeDistance >= mastery.ApplyAmount;
end
function ModifyAbilityByMastery_Module_HighPressureNozzle(ability, owner, mastery)
	local applyAmount = mastery.ApplyAmount2;
	if owner.Info.name == 'Drone_Flamethrower' then
		applyAmount = applyAmount + mastery.ApplyAmount3;
	end
	ability.TargetRange = GetExpandRangeAmount(ability.TargetRange, applyAmount);
	-- 적용 범위 증가 (자신 주위 광역 or 부채꼴 or 직선)
	local applyRangeCls = GetClassList('Range')[ability.ApplyRange];
	if ability.Target == 'Self' or applyRangeCls.Type == 'Fan' or applyRangeCls.Type == 'StraightLine' then
		ability.ApplyRange = GetExpandRangeAmount(ability.ApplyRange, applyAmount);
		ability.GuideRange = GetExpandRangeAmount(ability.GuideRange, applyAmount);
		-- 지형효과 적용 범위 증가
		for _, info in ipairs(ability.ApplyFieldEffects) do
			info.Range = GetExpandRangeAmount(info.Range, applyAmount);
		end
	end
end
-- 보조 술법 개량
function ModifyAbilityChecker_ImprovedJutsu(mastery, ability, owner)
	return ability.Category == 'Ninjutsu' and (ability.Type == 'Assist' or ability.Type == 'Heal');
end
function ModifyAbilityByMastery_ImprovedJutsu(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	...
	-- 시전 지연 시간
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * mastery.ApplyAmount3 / 100));
end
-- 회복술 강화
function ModifyAbilityChecker_HealJutsu(mastery, ability, owner)
	return ability.Category == 'Ninjutsu' and ability.Type == 'Heal';
end
function ModifyAbilityByMastery_HealJutsu(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	if ability.SubType2 == 'HP' then
		local applyAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2;
		ability.ApplyAmountChangeStep[1] = ability.ApplyAmountChangeStep[1] + math.floor(host.ApplyAmountChangeStep[1] * applyAmount / 100);
	elseif ability.SubType2 == 'Cost' then
		local applyAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount3) * mastery.ApplyAmount4;
		ability.ApplyCost = ability.ApplyCost + math.floor(host.ApplyCost * applyAmount / 100);
	end
end
-- 차크라 개방
function ModifyAbilityChecker_ReleaseChakra(mastery, ability, owner)
	return ability.Category == 'Ninjutsu' and ability.IsUseCount;
end
function ModifyAbilityByMastery_ReleaseChakra(ability, owner, mastery)
	-- 사용 제한 횟수 증가
	ability.MaxUseCount = ability.MaxUseCount + mastery.ApplyAmount;
end
-- 짙은 그림자술
function ModifyAbilityChecker_DeepShadowNinjutsu(mastery, ability, owner)
	return Set.new({'NinjutsuShadow_Stealth', 'NinjutsuShadow_Trace', 'NinjutsuShadow_Illusion'})[ability.name];
end
function ModifyAbilityByMastery_DeepShadowNinjutsu(ability, owner, mastery)
	-- 버프 턴 설정이 없으면 기본 값 세팅
	if ability.ApplyTargetBuffDuration == 0 then
		local host = ability;
		if not IsClass(ability) then
			host = GetHostClass(ability);
		end
		ability.ApplyTargetBuffDuration = host.ApplyTargetBuff.Turn;
	end
	-- 버프 턴 증가
	ability.ApplyTargetBuffDuration = math.max(0, ability.ApplyTargetBuffDuration + mastery.ApplyAmount);
	ability.ApplyTargetBuff.Turn = ability.ApplyTargetBuffDuration;
...
-- 탁류술
function ModifyAbilityChecker_DeepWaterNinjutsu(mastery, ability, owner)
	return Set.new({'NinjutsuWater_WaterWave', 'NinjutsuWater_WaterFall', 'NinjutsuWater_HeavyRain'})[ability.name];
end
function ModifyAbilityByMastery_DeepWaterNinjutsu(ability, owner, mastery)
	ability.ApplyTargetBuff = GetClassList('Buff')[mastery.Buff.name];
end
-- 난도질 연마
function ModifyAbilityChecker_ImprovedSliceAndDice(mastery, ability, owner)
	return ability.Type == 'Attack' and (ability.RandomPickCount > 0 or ability.RepeatCount > 0);
end
function ModifyAbilityByMastery_ImprovedSliceAndDice(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 의료 닌자
function ModifyAbilityChecker_MedicNinja(mastery, ability, owner)
	return ability.Category == 'Ninjutsu' and ability.Type == 'Heal';
end
function ModifyAbilityByMastery_MedicNinja(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 그림자 닌자
function ModifyAbilityChecker_ShadowNinja(mastery, ability, owner)
	return ability.name == 'NinjutsuShadow';
end
function ModifyAbilityByMastery_ShadowNinja(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 안개 닌자
function ModifyAbilityChecker_MistNinja(mastery, ability, owner)
	return ability.name == 'NinjutsuMist';
end
function ModifyAbilityByMastery_MistNinja(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	...
	-- 시전 지연 시간 감소
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * mastery.ApplyAmount4 / 100));
end
-- 수둔 닌자
function ModifyAbilityChecker_WaterNinja(mastery, ability, owner)
	return ability.name == 'NinjutsuWater' or Set.new({'NinjutsuWater_WaterWave', 'NinjutsuWater_WaterFall', 'NinjutsuWater_HeavyRain'})[ability.name];
end
function ModifyAbilityByMastery_WaterNinja(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	if ability.name == 'NinjutsuWater' then
		-- 사용 제한 횟수 증가
		if ability.IsUseCount then
			ability.MaxUseCount = ability.MaxUseCount + mastery.ApplyAmount;
		end
		-- 공격 가능 거리 증가
		ability.TargetRange = GetExpandRangeAmount(ability.TargetRange, mastery.ApplyAmount2);
		-- 재사용 대기 시간
		ability.CoolTime = math.max(0, ability.CoolTime - mastery.ApplyAmount3);
		-- 시전 지연 시간 감소
		ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * mastery.ApplyAmount4 / 100));
	else
		-- 기본 피해량 증가
		ability.ApplyAmountChangeStep[1] = ability.ApplyAmountChangeStep[1] + math.floor(host.ApplyAmountChangeStep[1] * mastery.ApplyAmount5 / 100);
	end
end
-- 암기 전문가
function ModifyAbilityChecker_AmbushNinja(mastery, ability, owner)
	return GetAbilityItemSlot(ability, owner) == 'NinjaToolkit';
end
-- 인법 닌자
function ModifyAbilityChecker_NinjutsuNinja(mastery, ability, owner)
	return ability.Category == 'Ninjutsu';
end
function ModifyAbilityByMastery_NinjutsuNinja(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 재빠른 썰기
function ModifyAbilityChecker_QuickSlashing(mastery, ability, owner)
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.HitRateType == 'Melee' and ability.CastDelay > 0;
end
function ModifyAbilityByMastery_QuickSlashing(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 재빠른 양손 베기
function ModifyAbilityChecker_QuickTwoHandSlashing(mastery, ability, owner)
	local weaponType = SafeIndex(owner, 'Weapon', 'Type', 'name');
	local weaponTypeSet = Set.new({'TwoHandAxe', 'TwoHandSpear', 'TwoHandSword'});
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.HitRateType == 'Melee' and weaponTypeSet[weaponType];
end
function ModifyAbilityByMastery_QuickTwoHandSlashing(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 발도술
function ModifyAbilityChecker_QuickdrawSword(mastery, ability, owner)
	return ability.Type == 'Attack' and ability.SubType == 'Slashing' and ability.CastDelay > 0;
end
function ModifyAbilityByMastery_QuickdrawSword(ability, owner, mastery)
	local additiveEffect = {};
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	...
	local applyAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2;
	-- 활인검 강화
	local mastery_HumanSword = GetMasteryMastered(GetMastery(owner), 'HumanSword');
	if mastery_HumanSword then
		applyAmount = applyAmount + math.floor(mastery_HumanSword.CustomCacheData / mastery_HumanSword.ApplyAmount) * mastery_HumanSword.ApplyAmount2;
		table.insert(additiveEffect, mastery_HumanSword);
	end
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * applyAmount / 100));
	return unpack(additiveEffect);
end
-- 정령 친화
function ModifyAbilityChecker_ElementalHarmony(mastery, ability, owner)
	return IsGetAbilitySubType(ability, 'ESP') and ability.CastDelay > 0;
end
function ModifyAbilityByMastery_ElementalHarmony(ability, owner, mastery)
	local additiveEffect = {};
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	...
	local applyAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2;
	-- 정령 교감
	local mastery_ElementalCommunion = GetMasteryMastered(GetMastery(owner), 'ElementalCommunion');
	if mastery_ElementalCommunion then
		applyAmount = applyAmount + math.floor(mastery_ElementalCommunion.CustomCacheData / mastery_ElementalCommunion.ApplyAmount3) * mastery_ElementalCommunion.ApplyAmount4;
		table.insert(additiveEffect, mastery_ElementalCommunion);
	end
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * applyAmount / 100));
	return unpack(additiveEffect);
end
-- 헌신
function ModifyAbilityChecker_Devotion(mastery, ability, owner)
	return (ability.Type == 'Heal' or ability.Type == 'Assist') and ability.CoolTime > 0;
end
function ModifyAbilityByMastery_Devotion(ability, owner, mastery)
	-- 재사용 대기 시간
	ability.CoolTime = math.max(0, ability.CoolTime - mastery.ApplyAmount);
end
-- 총기손질
function ModifyAbilityChecker_GunMaintenance(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Piercing' and ability.HitRateType == 'Force' and (GetWithoutError(ability.AdditionalApplyAmount, 'AttackPower') or ability.CastDelay > 0);
end
function ModifyAbilityByMastery_GunMaintenance(ability, owner, mastery)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 새로운 사용법
function ModifyAbilityChecker_NewDirection(mastery, ability)
	return ability.IsUseCount and ability.AutoUseCount and ability.MaxUseCount > 0;
end
function ModifyAbilityByMastery_NewDirection(ability, owner, mastery)
	ability.MaxUseCount = ability.MaxUseCount + mastery.ApplyAmount;
end
-- 영리한 손놀림
function ModifyAbilityChecker_CleverFinger(mastery, ability)
	return ability.CastDelay > 0;
end
function ModifyAbilityByMastery_CleverFinger(ability, owner, mastery)
	local additiveEffect = {};
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	...
	local applyAmount = math.floor(mastery.CustomCacheData / mastery.ApplyAmount) * mastery.ApplyAmount2;
	-- 영리한 야수
	local mastery_CleverBeast = GetMasteryMastered(GetMastery(owner), 'CleverBeast');
	if mastery_CleverBeast then
		applyAmount = applyAmount + math.floor(mastery_CleverBeast.CustomCacheData / mastery_CleverBeast.ApplyAmount) * mastery_CleverBeast.ApplyAmount2;
		table.insert(additiveEffect, mastery_CleverBeast);
	end
	ability.CastDelay = math.max(0, ability.CastDelay - (host.CastDelay * applyAmount / 100));
	return unpack(additiveEffect);
end
-- 모방 기억
function ModifyAbilityByMastery_ImitationMemory(ability, owner, mastery)
	ability.MaxUseCount = ability.MaxUseCount + mastery.ApplyAmount3;
end
-- 진홍까마귀 단검
function ModifyAbilityChecker_Dagger_CrimsonCrow(mastery, ability)
	return ability.CoolTime > 0;
end
-- 붐 스프레이 SHD-40S
function ModifyAbilityChecker_Brush_Boom_SHD_Epic(mastery, ability)
	return Set.new({'SprayHeal', 'SprayVigor', 'SprayCharging'}):has(ability.name);
end
-- 가스 스프레이 SHD-40S
function ModifyAbilityChecker_Brush_Green_SHD_Epic(mastery, ability)
	return Set.new({'SprayHeal', 'SprayVigor', 'SprayCharging'}):has(ability.name);
end
-- 만능 스프레이 FPU
function ModifyAbilityChecker_Brush_AllPowerful_FPU_Legend(mastery, ability)
	return Set.new({'SprayHeal', 'SprayVigor', 'SprayCharging'}):has(ability.name);
end
-- 아웃포스 M1
function ModifyAbilityChecker_Pistol_OutForceM1(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Piercing' and ability.HitRateType == 'Force' and ability.TargetType == 'Single' and ability.CastDelay > 0;
end
function ModifyAbilityChecker_Pistol_OutForceM1_Rare(mastery, ability)
	return ModifyAbilityChecker_Pistol_OutForceM1(mastery, ability);
end
function ModifyAbilityChecker_Pistol_OutForceM1_Epic(mastery, ability)
	return ModifyAbilityChecker_Pistol_OutForceM1(mastery, ability);
end
-- 아웃포스 X
function ModifyAbilityChecker_Pistol_OutForceM1_Legend(mastery, ability)
	return ability.Type == 'Attack' and ability.SubType == 'Piercing' and ability.HitRateType == 'Force' and ability.TargetType == 'Single';
end
function ModifyAbilityByMastery_Pistol_OutForceM1_Legend(ability, owner, mastery)
	ability.TargetRange = GetExpandRangeAmount(ability.TargetRange, mastery.ApplyAmount);
end
-- 멍고! 멍고!! 멍고!!!
function ModifyAbilityChecker_MunggoMunggoMunggo(mastery, ability)
	return ability.name == 'Munggo_HappyMode' or ability.name == 'Munggo_RageMode';
end
---@param ability class_Ability
function ModifyAbilityByMastery_MunggoMunggoMunggo(ability, owner, mastery)
	if ability.name == 'Munggo_HappyMode' then
		ability.ApplyTargetBuff = GetClassList('Buff')[mastery.SubBuff.name];
		ability.RemoveBuff = GetClassList('Buff')[mastery.ForthBuff.name];
	elseif ability.name == 'Munggo_RageMode' then
		ability.ApplyTargetBuff = GetClassList('Buff')[mastery.ForthBuff.name];
		ability.RemoveBuff = GetClassList('Buff')[mastery.SubBuff.name];
	end
end
-- 파스칼의 작업용 손목 고리
function ModifyAbilityChecker_Bracelet_Pascal(mastery, ability)
	return ability.IsUseCount and ability.AutoUseCount and ability.MaxUseCount > 0;
end
function ModifyAbilityByMastery_Bracelet_Pascal(ability, owner, mastery)
	ModifyAbilityByMastery_MaxUseCount(ability, owner, mastery);
end
-- 붐 스프레이 FUHD-40S
function ModifyAbilityChecker_Brush_Boom_FUHD_Epic(mastery, ability)
	return Set.new({'ShakeShot_Wind', 'ShakeShot_Wind2'}):has(ability.name);
end
-- 화염 스프레이 FUHD-40S
function ModifyAbilityChecker_Brush_Red_FUHD_Epic(mastery, ability)
	return Set.new({'ShakeShot_Red', 'ShakeShot_Red2'}):has(ability.name);
end
-- 빙결 스프레이 FUHD-40S
function ModifyAbilityChecker_Brush_White_FUHD_Epic(mastery, ability)
	return Set.new({'ShakeShot_White', 'ShakeShot_White2'}):has(ability.name);
end
-- 가스 스프레이 FUHD-40S
function ModifyAbilityChecker_Brush_Green_FUHD_Epic(mastery, ability)
	return Set.new({'ShakeShot_Green', 'ShakeShot_Green2'}):has(ability.name);
end
-- 보조 라디에이터
function ModifyAbilityChecker_Module_SubRadiator(mastery, ability)
	return ability.CoolTime > 0;
end
-- 향상된 가속 추진기
function ModifyAbilityChecker_Module_EnhancedBoosts_Speed(mastery, ability)
	-- 버프 강화로 처리됨
end
-- 향상된 추가 부스터
function ModifyAbilityByMastery_Module_ImprovedAdditionalBooster(ability, owner, mastery)
	local applyAmount = mastery.ApplyAmount;
	if IsMissionServer() then
		local masterKey = GetInstantProperty(owner, 'SummonMaster');
		local master = GetUnit(owner, masterKey);
		if SafeIndex(master, 'Job', 'name') == 'Engineer' then
			applyAmount = applyAmount + mastery.ApplyAmount;
		end
	end
	ability.MaxUseCount = ability.MaxUseCount + applyAmount;
end
------------------------------------------------------
-- 버프 어빌리티 조정자
-------------------------------*BUFF_ABILITY_MODIFIERS
-- 과열
function ModifyAbilityChecker_Buff_Overdrive(buff, ability, owner)
	return ability.Cost > 0;
end
function ModifyAbilityByBuff_Overdrive(ability, owner, buff)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	ability.Cost = math.max(0, ability.Cost + (host.Cost * buff.ApplyAmount / 100));
end
-- 예열
...
-- 냉정
function ModifyAbilityChecker_Buff_DeadlyCalm(buff, ability, owner)
	return ability.CastDelay > 0;
end
function ModifyAbilityByBuff_DeadlyCalm(ability, owner, buff)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 육도윤회
function ModifyAbilityChecker_Buff_SixPathSamsara(buff, ability, owner)
	return ability.Category == 'Ninjutsu' and ability.CoolTime >= 0;
end
function ModifyAbilityByBuff_SixPathSamsara(ability, owner, buff)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
...
-- 하늘의 눈
function ModifyAbilityChecker_Buff_DistanceSnipeMode(buff, ability, owner)
	return ability.Type == 'Attack' and ability.HitRateType == 'Force' and ability.TargetType == 'Single' and IsGetAbilitySubType(ability, 'Piercing');
end
function ModifyAbilityByBuff_DistanceSnipeMode(ability, owner, buff)
	ability.TargetRange = GetExpandRangeAmount(ability.TargetRange, buff.ApplyAmount);
end
-- 광활한 하늘의 눈
function ModifyAbilityChecker_Buff_DistanceSnipeMode2(buff, ability, owner)
	return ModifyAbilityChecker_Buff_DistanceSnipeMode(buff, ability, owner);
end
function ModifyAbilityByBuff_DistanceSnipeMode2(ability, owner, buff)
	ModifyAbilityByBuff_DistanceSnipeMode(ability, owner, buff);
end
------------------------------------------------------
-- 아이템 어빌리티 조정자
------------------------------------------------------
...
function IsProtocolAbility(ability)
	return ability.Category == 'Protocol';
end
function IsCommandAbilityProtocolAbility(ability)
	return ability.Category == 'Protocol' and ability.AbilitySubMenu ~= '';
end
------------------------------------------------------
-- 어빌리티 목록 정렬
------------------------------------------------------
function SortAbilityList(target, abilityList, abilityGetter)
	...
	abilityAllList = table.filter(abilityAllList, function(ability) return SafeIndex(ability, 'name') ~= nil end);
	-- 기본 순서
	local baseOrderMap = {};
...
function SetAbilityTypeData(ability)
	...
	if ability.SubType ~= 'None' then
		...
		if ability.Type == 'Attack' then
			...
			table.insert(propertyList, { Title = 'DamageType2', Value = abilitySubType });
		end
	end
	-- 10. HitRateType 표기
	...
	-- 15. 발동 범위 표기
	if ability.Type == 'Trap' then
		table.insert(propertyList, { Title = 'ApplyRadius', Value = ability.RangeRadius, Font = 'HemiHead-14' });
	elseif ability.SubType2 == 'Gas' then
		table.insert(propertyList, { Title = 'ApplyRadius', Value = ability.RangeRadius, Font = 'HemiHead-14' });
	elseif ability.RangeRadius > 0 then
		-- 14. 공격 범위 표기
	...
	for index, data in ipairs (abilityTypeList) do
		...
		if data.Value and data.Value ~= '-' then
			if index == 1 then
				...
				if data.Title == 'AttackRange' or data.Title == 'AttackRadius' or data.Title == 'ApplyRadius' then
...
-- AbilitySubMenu와 detailInfo에 의한 서브 어빌리티
function GetSubCommandAbility(self, ability, detailInfo)
	if ability.AbilitySubMenu == 'DetailedSnipe' or ability.AbilitySubMenu == 'HackingProtocol' then
		-- 아무 것도 안함
	elseif IsCommandAbilityProtocolAbility(ability) then
		local protocolDetail = SafeIndex(detailInfo, 'ProtocolDetail');
		if protocolDetail then
			local protocolDetailCls = GetClassList(ability.AbilitySubMenu)[protocolDetail];
			if protocolDetailCls then
				local subAbility = GetAbilityObject(self, protocolDetailCls.Ability.name);
				if subAbility then
					ability = subAbility;
				end
			end
		end
	elseif ability.AbilitySubMenu ~= '' then
		local protocolDetail = SafeIndex(detailInfo, 'SubCommand');
		if protocolDetail then
			local protocolDetailCls = GetClassList(ability.AbilitySubMenu)[protocolDetail];
			if protocolDetailCls then
				local subAbility = GetAbilityObject(self, protocolDetailCls.Ability.name);
...
function StashAbilityForApplyModifier(ability)
	return {ability.Cool, ability.UseCount, ability.PerformanceEffect, ability.Active};
end
function ApplyAbilityStashedData(ability, stashData)
	ability.Cool = stashData[1];
	ability.UseCount = stashData[2];
	ability.PerformanceEffect = stashData[3];
	ability.Active = stashData[4];
end