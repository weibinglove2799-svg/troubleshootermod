...
| **免疫精神异常** | 本 MOD |
| **高度增益：每登高 1 格高度（游戏内高度单位，15 高度值 = 1 层），所有技能最大距离 +1、视野 +1；登高/下坡移动时实时同步** | 本 MOD |
...
- **射程无视视野**：按技能射程直接判定，视野外的敌人只要距离在射程内同样会被反击（射程含高度增益：角色每登高 1 格高度，反击射程同步 +1）；
...
- `MODWORK/Data/script/server/mastery.lua` — 个人主义关联、预热与附带天赋的分发（含高度增益同步时机）；
- `MODWORK/Data/script/shared/shared_ability.lua` — 预热高度增益：每登高 1 层技能射程 +1、视野 +1（`Xzfj_SyncHeightEffects`），施法时按高度扩展技能射程；
