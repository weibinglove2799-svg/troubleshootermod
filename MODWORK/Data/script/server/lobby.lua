...
function LOBBY_ENTER(company, lobbyType, dc)
	...
	if rosterCount == 0 or pcIndexSet[company.LeaderID] == nil then
		...
		dc:UpdateCompanyProperty(company, 'LeaderID', leaderID);
	end	
...
function LobbyAction_TechniqueResearch(dc, company, args)
	...
	local autoSubMastery = args.AutoSubMastery;
	...
	local isEnableTechnique, masteryList, reason, _, itemList, craftList = IsEnableTechniqueResearch(company, tech, addMasteries, itemCounter, count, autoSubMastery);
	...
	for itemKey, needCount in pairs(itemList) do
		TakeInventoryItemByType(dc, company, itemKey, needCount, true);
	end
	...
	-- 5. 일괄 제작 시, 하위 재료 제작으로 인한 제작 숙련도/레시피 언락 처리
	local unlockRecipeList = {};
	if autoSubMastery then
		local addExpMap = {};
		for itemName, itemCount in pairs(craftList) do
			local _, addExpInfos = GetCraftAddExpList(company, itemName, itemCount);
			for _, info in ipairs(addExpInfos) do
				addExpMap[info.Recipe.name] = (addExpMap[info.Recipe.name] or 0) + info.AddExp;
			end
		end
		for recipeName, addExp in pairs(addExpMap) do
			local unlockRecipe = AddRecipeExp(dc, company, recipeName, addExp);
			table.append(unlockRecipeList, unlockRecipe);
		end
	end
	
	SendNotification(company, 'ResearchTechniqueCompleted', 
		{ Type = tech.name, MasteryList = masteryList, Techniques = unlockTechniques, ItemType = tech.RewardItem, ItemCount = itemRewardCount, UnlockRecipe = unlockRecipeList}
	);
	return {Success = true, Reward = masteryList};
end

function LobbyInventoryItemCounter(owner)
	return function(itemName, includeWarehouse)
		local ret = 0;
		local item = GetInventoryItemByType(owner, itemName);
		if item then
			ret = ret + item.Amount;
		end
		if includeWarehouse then
			local item2 = GetWarehouseItemByType(owner, itemName);
			if item2 then
				ret = ret + item2.Amount;
			end
		end
		return ret;
	end;
end
function LobbyRosterGetter(company)
	return function(rosterName)
		return GetRoster(company, rosterName);
	end
end
function LobbyEquipItemChecker(owner, rosterType)
	local rosterList = GetAllRoster(owner, rosterType or 'All');
	return function(itemName, equipSlot)
		local itemSlotList = equipSlot and { equipSlot } or GetItemSlotList();
		for _, roster in ipairs(rosterList) do
			for index = 0, roster.EquipmentSlot.Count - 1 do
				for _, itemSlot in ipairs(itemSlotList) do
					local curEquip = GetRosterEquipItem(roster, itemSlot, index);
					if curEquip ~= nil and curEquip.name == itemName then
						return true, roster, index;
					end
				end
			end
		end
...
---@param includeWarehouse boolean
function TakeInventoryItemByType(dc, company, itemType, itemCount, includeWarehouse)
	if includeWarehouse then
		-- 인벤토리에서는 개수 맞춰서 처리
		local invItem = GetInventoryItemByType(company, itemType);
		if invItem and invItem.Amount > 0 then
			local invCount = invItem.Amount;
			---@cast invCount integer
			local takeCount = math.min(invCount, itemCount);
			itemCount = itemCount - takeCount;
			dc:TakeItem(invItem, takeCount);
		end
		-- 창고에서는 모자르면 내부적으로 실패
		if itemCount > 0 then
			local warehouseItem = GetWarehouseItemByType(company, itemType);
			dc:TakeItemWarehouse(warehouseItem, itemCount);
		end
	else
		-- 개수 모자르면 내부적으로 실패
		local invItem = GetInventoryItemByType(company, itemType);
		dc:TakeItem(invItem, itemCount);
	end	
end
-------------------------------------------------------------------------------
-- 아이템 강화
...
function LobbyAction_UpgradeItem(dc, company, args)
	...
	for index, mt in ipairs (materialList) do
		TakeInventoryItemByType(dc, company, mt.Item, mt.Amount, true);
...
function LobbyAction_DismantleItem(dc, company, args)
	...
	for _, item in ipairs(items) do
		...
		for index, material in ipairs (materialList) do
			reward[material.Item] = (reward[material.Item] or 0) + material.Amount;
		end
	end
...
function LobbyAction_CraftItem(dc, company, args)
	...
	local autoSubMaterial = args.AutoSubMaterial;
	...
	local enableCraft, reason, itemList, craftList = IsEnableCraftItem(company, roster, item, itemCount, LobbyInventoryItemCounter(company), nil, autoSubMaterial);
	if not enableCraft then
		for key, value in ipairs (reason) do
			LogAndPrint('[Carft Failed] reason - '..key..': '..value);
		end
		return {Success = false, Reward = {}, UnlockRecipe = unlockRecipeList, Exp = 0 };
	end
	...
	for itemKey, needCount in pairs(itemList) do
		TakeInventoryItemByType(dc, company, itemKey, needCount, true);
		table.insert(reward, {ItemName = itemKey, ItemCount = needCount});
	end
	dc:GiveItem(company, itemName, itemCount);

	-- 4. 숙련도 + 오픈.
	local addExp, addExpInfos = GetCraftAddExpList(company, itemName, itemCount);
	for _, info in ipairs(addExpInfos) do
		local unlockRecipe = AddRecipeExp(dc, company, info.Recipe.name, info.AddExp);
		table.append(unlockRecipeList, unlockRecipe);
	end
	-- 5. 일괄 제작 시, 하위 재료 제작으로 인한 제작 숙련도/레시피 언락 처리
	if autoSubMaterial then
		local addExpMap = {};
		for itemName, itemCount in pairs(craftList) do
			local _, addExpInfos = GetCraftAddExpList(company, itemName, itemCount);
			for _, info in ipairs(addExpInfos) do
	...
	return {Success = true, Reward = reward, UnlockRecipe = unlockRecipeList, Exp = addExp };
end
-------------------------------------------------------------------------------
-- 아이템 추출
-------------------------------------------------------------------------------
...
function LobbyAction_ExtractItem(dc, company, args)
	...
	if item == nil then
		LogAndPrint('[Extract Failed] 대상 아이템을 찾지 못했다. company : ', company.CompanyName, ', itemData : ', args.BaseInvKey);
		return {Success = false, Reward = {}};
	end
...
function LobbyAction_IdentifyItems(dc, company, args)
	...
	if company.Vill < totalPrice then
		LogAndPrint('[Identify Failed] 돈이 부족하다. company : ', company.name, ', Vill : ', company.Vill, ', Price : ', totalPrice);
		return {Success = false, LevelUp = false, Price = 0, FriendshipChange = 0};
	end
...
function LobbyAction_AddMastery(dc, company, args)
	...
	local isEnable, reason = MasteryTrainingTest(roster, company, masteryTable, masteryName, false);
	if not isEnable then
		---@cast reason table
	...
	for _, setMastery in ipairs(GetClassList('Mastery')[masteryName].EnableSetMasteries) do
		if not company.MasterySetIndex[setMastery.name] then
			-- [MOD] 附加天赋效果信息解锁条件放宽：
			-- 原版要求集齐套装全部子天赋才解锁图鉴信息；现改为已装备子天赋达到
			-- XZJF_SetMasteryMinCount 个即解锁（与生效、界面显示阈值一致，见
			-- shared_mastery.lua 顶部常量说明）。DATA 成品默认 1（单天赋即解锁）。
			local needCount = 0;
			local hasCount = 0;
			local index = 1;
			while true do
				...
				if m ~= 'None' then
					...
					if m == masteryName or GetMasteryMastered(masteryTable, m) then
						hasCount = hasCount + 1;
					end
				end
				index = index + 1;
...
function SharedExtractMasteryAction(dc, company, roster, extractMasteries, noCost)
	...
	for _, masteryName in ipairs(extractMasteries) do
		...
		-- 3) 필요 아이템 소모
		if not noCost and curMastery.Cost > 0 and not StringToBool(company.CheatState.NoUseManual) then
		...
		if Set.new({'AlchemyBag', 'GrenadeBag', 'DoubleGear', 'NinjaToolkit', 'Module_AuxiliaryWeapon', 'Module_AssistEquipment'})[masteryName] then
...
function LobbyAction_ExtractMasteryAll(dc, company, args)
	...
	local itemSlotExtentions = Set.new({'AlchemyBag', 'GrenadeBag', 'DoubleGear', 'NinjaToolkit', 'Module_AuxiliaryWeapon', 'Module_AssistEquipment'});
	for _, mastery in ipairs(masteryList) do
...
function LobbyAction_TroublemakerReward(dc, company, args)
	...
	if unlockRecipeList then
		for _, recipeName in ipairs(unlockRecipeList) do
			UpdateUnlockRecipe(dc, company, recipeName);
		end
	end
	return {Success = true, Reward = reward, UnlockTechList = unlockTechList, UnlockRecipeList = unlockRecipeList};
end
function LobbyAction_TroublemakerRewardAll(dc, company, args)
	local isEnable, monList, rewardList, unlockTechList, unlockRecipeList = TroublemakerRewardAllTest(company);
	if not isEnable then
		return {Success = false, Reward = rewardList, UnlockTechList = unlockTechList, UnlockRecipeList = unlockRecipeList};
	end
	-- 1) 보상 완료 프로퍼티 갱신
	for _, monName in pairs(monList) do
		dc:UpdateCompanyProperty(company, string.format('Troublemaker/%s/Reward', monName), true);
	end
	-- 2) 보상 아이템 주기.
	for _, info in pairs(rewardList) do
		dc:GiveItem(company, info.Type, info.Count, true, '_INV_FULL_TROUBLEMAKER_REWARD_');
	end
	-- 3) 특성 연구 언락
	for _, techName in ipairs(unlockTechList) do
		dc:UpdateCompanyProperty(company, string.format('Technique/%s/Opened', techName), true);
	...
	return {Success = true, Reward = rewardList, UnlockTechList = unlockTechList, UnlockRecipeList = unlockRecipeList};
end
...
function LobbyAction_ChangeCharacterMastery(dc, company, args)
	...
	for _, masteryName in ipairs(extractMasteries) do
		...
		if mastery and mastery.ExtractItem ~= 'None' then
			...
			dc:UpdateMasteryLv(roster, masteryName, 0, roster.MasteryBoard.Index);
...
function LobbyAction_InvokeClientEvent(dc, company, args)
	...
	return {Success = handlerFunc(company, args.EventArgs)}
end
...
function LobbyAction_NewMasteryConfirmed(dc, company, args)
	...
	for i, masteryName in ipairs(masteries) do
		if company.Mastery[masteryName].IsNew then
			dc:UpdateCompanyProperty(company, string.format('Mastery/%s/IsNew', masteryName), false);
		elseif company.Mastery[masteryName].IsAdd then
			dc:UpdateCompanyProperty(company, string.format('Mastery/%s/IsAdd', masteryName), false);
		end
	end
	return {Success = true};
end
...
function ResetActivityReport(dc, company)
	...
	-- 5. 합동훈련 보상 회수 초기화
	dc:UpdateCompanyProperty(company, 'ActivityReport/JointTrainingCounter', 0);
end
-------------------------------------------------------------------------
-- 트러블북 보상받기.
-------------------------------------------------------------------------
...
function LobbyAction_UpdateDifficulty(dc, company, args)
	...
	local gameDifficultyCls = gameDifficultyClsList[args.Difficulty];
	if gameDifficultyCls == nil then
		return {Success = false};
	end
	if gameDifficultyCls.SinglePlayOnly and not IsSingleplayMode() then
		return {Success = false};
	end	
...
---@param company company
---@param args table
function LobbyAction_UpdateSecondWind(dc, company, args)
	for secondWindType, enabled in pairs(args.SecondWind) do
		(function()
			if secondWindType == 'HighRiskReturn' and company.Progress.Tutorial.HighRiskReturn == 0 then
				return;
			end
			local prevEnabled = SafeIndex(company, 'SecondWind', secondWindType);
			if prevEnabled ~= nil and prevEnabled ~= enabled then
				dc:UpdateCompanyProperty(company, 'SecondWind/'..secondWindType, enabled);
			end
		end)();
	end

	for rosterKey, extractTargets in pairs(args.ExtractMasteryList) do
		local roster = GetRoster(company, rosterKey);
		if roster then
			for _, mastery in ipairs(extractTargets) do
				-- 1) 로스터 마스터리 레벨 초기화
				dc:UpdateMasteryLv(roster, mastery, 0);
				-- 2) 회사 마스터리 카운트 증가
				dc:AcquireMastery(company, mastery, 1, true);
			end
		end
	end
	return {Success = true};
		...
		if currencyAmount >= totalPrice then
			...
			for shopIndex, count in pairs(args.Cart) do
				if items[shopIndex].GoodsType == 'Recipe' then
					...
					UpdateUnlockRecipe(dc, company, items[shopIndex].Recipe);
				else
...
function LobbyAction_FoodShopAction(dc, company, args)
	...
	if args.Action == 'Buy' then -- 구매
		...
		local isEnable, totalPrice, reason, isRecommendMenu = IsEnableFoodOrder(company, menus, orderList, rosterList);
		local currencyAmount = GetCurrencyAmount(company, currency);
		if currencyAmount < totalPrice then
			isEnable = false;
		end
		...
		local guideTrigger = {};
		if setEffect then
			dc:UpdateCompanyProperty(company, 'FoodSet/'..setEffect.name, true);
		end
		if hasFoodSetEffect then
			...
			-- 시온 영입 이후
			local pc = GetRoster(company, 'Sion');
			if pc then
				UpdateRosterPersonality(dc, pc, 'Sion_GoodFoodTogether', nil, true);
			end
		end
		UpdateCurrencyAmount(dc, company, foodShopCls.Currency, - totalPrice);
		local addFriendship = 0;
		if shopHost then
			local friendshipPrice = totalPrice;
			if isRecommendMenu then
				friendshipPrice = friendshipPrice * 2;
			end		
			addFriendship = AddFriendshipByTradingPrice(dc, company, shopHost, friendshipPrice, response, function(newFriendship)
		...
		-- 시온 맛있는 음식은 모두와 함께~
		local pc = GetRoster(company, 'Sion');
		if pc and setEffect and rosterSet[pc.name] then
			IfHavePersonality(pc, 'Sion_GoodFoodTogether', function(personality)
				local addList = {};
				for _, target in ipairs(rosters) do
					if pc ~= target and newSetEffectSet[target.name] then
						table.insert(addList, { From = pc, To = target, AddPoint = personality.ApplyAmount });
					end
				end
				AddPCFriendshipPointMultiple(dc, addList);
				dc:Commit('FoodShopBuy:'..args.ShopType);
			end);
		end		
	elseif args.Action == 'Refresh' then		-- 아이템 갱신
...
function LobbyAction_ChangeMasteryBoard(dc, company, args)
	...
	local extractNeedCount = 0;	
	if not StringToBool(company.CheatState.NoUseManual) then
	...
	for masteryName, mastery in pairs(prevMasteryTable) do
		if mastery.Lv > 0 then
			local nextMastery = nextMasteryTable[masteryName];
			if not nextMastery or nextMastery.Lv <= 0 or removedMasteries[masteryName] then
	...
	if abilitySlotIndex and (roster.RosterType == 'Pc' or roster.RosterType == 'Beast') then
		...
		if #truncated > 0 then
			table.foreach(truncated, function(i, info) slotManager:Deactivate(abilityList[info.Ability]); end);
		end
...
function LobbyAction_CopyMasteryBoard(dc, company, args)
	local boardIndex = args.MasteryBoardIndex;
	if boardIndex == nil then
		LogAndPrint('LobbyAction_CopyMasteryBoard : boardIndex is nil!');
		return {Success = false, Item = '', Count = 0};
	end
	local roster = GetRoster(company, args.Roster);
	if roster == nil then
		LogAndPrint('LobbyAction_CopyMasteryBoard : roster is nil!');
		return {Success = false, Item = '', Count = 0};
	end
	local baseIndex = roster.MasteryBoard.Index;
	if baseIndex == boardIndex then
		LogAndPrint('LobbyAction_CopyMasteryBoard : Same boardIndex');
		return {Success = false, Item = '', Count = 0};
	end
	
	local itemCounter = LobbyInventoryItemCounter(company);
	local from = GetMastery(roster, baseIndex);
	local to = GetMastery(roster, boardIndex);
	
	local isEnable, reason, extractMasteryList, needItemCount, copyMasteryList, notEnoughList = MasteryBoardCopyTest(roster, company, from, to, itemCounter);
	if not isEnable then
		LogAndPrint('LobbyAction_CopyMasteryBoard : item is not enough!', needItemCount);
		return {Success = false, Item = '', Count = 0};
	end
	
	for _, mastery in ipairs(extractMasteryList) do
		-- 1) 로스터 마스터리 레벨 초기화
		dc:UpdateMasteryLv(roster, mastery.name, 0, boardIndex);
		-- 2) 회사 마스터리 카운트 증가
		dc:AcquireMastery(company, mastery.name, 1, true);
	end
	local extractNeedItem = '';
	local extractNeedCount = 0;
	for item, count in pairs(needItemCount) do
		extractNeedItem = item;
		extractNeedCount = extractNeedCount + count;
		-- 3) 필요 아이템 소모
		dc:TakeItem(GetInventoryItemByType(company, item), count);
	end
	
	-- 4) 필요한 특성 장착
	for _, mastery in ipairs(copyMasteryList) do
		-- 1) 로스터 마스터리 레벨 설정
		dc:UpdateMasteryLv(roster, mastery.name, 1, boardIndex);
		-- 2) 회사 마스터리 카운트 감소
		dc:LoseMastery(company, mastery.name, 1);
	end
...
-- 특성판 코드 불러오기
------------------------------------------------------
function LobbyAction_ImportMasteryCode(dc, company, args)
	local roster = GetRoster(company, args.Roster);
	if roster == nil then
		LogAndPrint('LobbyAction_ImportMasteryCode : roster is nil!');
		return {Success = false, Item = '', Count = 0};
	end
	local boardIndex = args.MasteryBoardIndex;
	if boardIndex == nil then
		LogAndPrint('LobbyAction_ImportMasteryCode : boardIndex is nil!');
		return {Success = false, Item = '', Count = 0};
	end
	
	local code = args.MasteryCode;
	local codeInfo = GetMasteryCodeInfo(code);
	local isValid = SafeIndex(codeInfo, 'Valid');
	if not isValid then
		local error = SafeIndex(codeInfo, 'Error');
		LogAndPrint('LobbyAction_ImportMasteryCode : code is invalid!', error);
		return {Success = false, Item = '', Count = 0};	
	end
	
	local isEnable, reason = IsEnableImportMasteryCode(roster, codeInfo);
	if not isEnable then
		LogAndPrint('LobbyAction_ImportMasteryCode : not enabled!', reason);
		return {Success = false, Item = '', Count = 0};		
	end
	...
	local isEnable, reason, extractMasteryList, needItemCount, equipMasteryList, notEnoughList, notEnableList = MasteryCodeImportTest(roster, company, codeInfo, masteryTable, itemCounter);
	if not isEnable then
		LogAndPrint('LobbyAction_ImportMasteryCode : item is not enough!', needItemCount);
		return {Success = false, Item = '', Count = 0};
	end
	
	for _, masteryName in ipairs(extractMasteryList) do
		-- 1) 로스터 마스터리 레벨 초기화
		dc:UpdateMasteryLv(roster, masteryName, 0, boardIndex);
		-- 2) 회사 마스터리 카운트 증가
		dc:AcquireMastery(company, masteryName, 1, true);
	end
	local extractNeedItem = nil;
	local extractNeedCount = 0;
	for item, count in pairs(needItemCount) do
		extractNeedItem = item;
		extractNeedCount = extractNeedCount + count;
	...
	-- 4) 필요한 특성 장착
	for _, masteryName in ipairs(equipMasteryList) do
		-- 1) 로스터 마스터리 레벨 설정
		dc:UpdateMasteryLv(roster, masteryName, 1, boardIndex);
		-- 2) 회사 마스터리 카운트 감소
		dc:LoseMastery(company, masteryName, 1);
	end
	
	return {Success = true, Item = extractNeedItem or '', Count = extractNeedCount};
end
------------------------------------------------------
-- 급여 변경
------------------------------------------------------
...
function LobbyAction_JukeBoxFavorites(dc, company, args)
	for _, info in ipairs(args.Favorites) do
		local bgmName = info.Name;
		local favorite = SafeIndex(company, 'JukeBoxFavorite', bgmName);
		if favorite ~= nil then
			dc:UpdateCompanyProperty(company, 'JukeBoxFavorite/'..bgmName, info.Favorite);
		end
	end
	return { Success = true };
end
...
function LobbyAction_EvolveBeast(dc, company, args)
	...
	if needCount and needCount > 0 then
		TakeInventoryItemByType(dc, company, needItem, needCount, true);
	end
	-- 2) 야수 진화
	...
	-- 유효하지 않은 장비 해제
	local mockObj = {Lv = roster.Lv};
	setmetatable(mockObj, {__index = beastTypeCls.Monster.Object});
	local checkItem = roster.Object.Weapon;
	if checkItem and checkItem.name and not IsEnableEquipItem(mockObj, checkItem, 'Weapon') then
		dc:UnequipItem(roster, 'Weapon', -1);
	end
	-- 빼야하는 특성 빼주기
	if #args.ExtractMasteries > 0 then
		SharedExtractMasteryAction(dc, company, roster, args.ExtractMasteries);
	end
	...
	if troublemaker and troublemaker.Exp < troublemaker.MaxExp then
		-- 최대 점수의 25% 획득
		local point = math.max(math.floor(troublemaker.MaxExp * 0.25), 1);
		-- 장벽 지구 보너스
		...
		if troublemaker.Exp + addPoint > troublemaker.MaxExp then
			addPoint = troublemaker.MaxExp - troublemaker.Exp;
		end
		dc:AddCompanyProperty(company, string.format('Troublemaker/%s/Exp', troublemaker.name), addPoint);
		if troublemaker.Exp == 0 then
			dc:UpdateCompanyProperty(company, string.format('Troublemaker/%s/IsNew', troublemaker.name), true);
		end
		SendNotification(company, 'AddTroublemakerExp', { Troublemaker = troublemaker.name, AddPoint = point });
	end
...
function ProgressEvolveBeastMasteryAction(ldm, self, company, env, parsedScript)
	...
	local targetStages = {};
	for i = 1, beastTypeCls.EvolutionStage do
		local evoM = roster['EvolutionMastery'..i];
		if evoM and evoM == 'None' then
			table.insert(targetStages, {Stage = i});
		end
	end
	...
	for i, stageInfo in ipairs(targetStages) do
		...
		if #candidateMasteries < 4 then
			...
			dc:Commit('ProgressEvolveBeastMasteryAction');
		end
		-- 다이얼로그
		local id, ok, result = ldm:Dialog('ScoutBeast', {BeastType = beastTypeCls.name, CandidateMasteries = candidateMasteries, TargetKey = roster.RosterKey, HelpInfo = helpInfo, EvolutionStage = stage, NeedInitialize = i == 1, NeedUpload = i == #targetStages});
		-- 진화 특성 반영
...
function LobbyAction_RepairAllMachine(dc, company, args)
	local totalPrice = 0;
	local rosters = {};
	local machineList = table.filter(GetAllRoster(company, 'Machine'), function(pcInfo) return not pcInfo.Stored; end);
	for _, roster in ipairs(machineList) do
		local isEnable, reason, repairPrice = IsEnableRepairMachine(company, roster, true);
		if isEnable then
...
function LobbyAction_StoreBeast(dc, company, args)
	...
	local isEnable = IsEnableStoreBeastOrMachine(company, roster);
	if not isEnable then
...
function LobbyAction_PickupBeast(dc, company, args)
	...
	local isEnable = IsEnablePickupBeastOrMachine(company, roster);
	if not isEnable then
...
function LobbyAction_SwapBeast(dc, company, args)
	...
	local isEnable = IsEnableSwapBeastOrMachine(company, roster, roster2);
	if not isEnable then
...
function LobbyAction_DeleteFormationPreset(dc, company, args)
	...
	local vSlotIndex = company.FormationPreset[slotIndex].SlotIndex;
	if vSlotIndex == 0 then
		vSlotIndex = slotIndex;
	end

	local reorderedFormationPreset = Linq.new(company.FormationPreset)
		:select(function(f, i) return {f, i, f.SlotIndex == 0 and i or f.SlotIndex}; end)	-- {FormationPreset, DBIndex, ViewIndex}
		:orderByAscending(function(d) return d[3] end)
		:toList();

	local fillUpIndex = vSlotIndex;
	local lastReplacedIndex = vSlotIndex;
	for i = fillUpIndex + 1, #company.FormationPreset do
		local targetFormationPreset = reorderedFormationPreset[i];
		if targetFormationPreset[1].Opened then
			local replaceSlot = reorderedFormationPreset[vSlotIndex]
			dc:UpdateCompanyProperty(company, 'FormationPreset/'..targetFormationPreset[2]..'/SlotIndex', replaceSlot[3]);
			vSlotIndex = vSlotIndex + 1;
			lastReplacedIndex = i;
		end
	end
	local baseSlot = reorderedFormationPreset[fillUpIndex];
	dc:UpdateCompanyProperty(company, 'FormationPreset/'..baseSlot[2]..'/SlotIndex', lastReplacedIndex);
...
function LobbyAction_ResetAbilityPreset(dc, company, args)
	local roster = GetRoster(company, args.Roster);
	if roster == nil then
		return {Success = false};
	end
	...
	local vSlotIndex = roster.AbilityPreset.Preset[slotIndex].Order;

	local reorderedAbilityPreset = Linq.new(roster.AbilityPreset.Preset)
		:select(function(f, i) return {f, i, f.Order}; end)	-- {FormationPreset, DBIndex, ViewIndex}
		:orderByAscending(function(d) return d[3] end)
		:toList();

	local fillUpIndex = vSlotIndex;
	local lastReplacedIndex = vSlotIndex;
	for i = fillUpIndex + 1, #roster.AbilityPreset.Preset do
		local targetFormationPreset = reorderedAbilityPreset[i];
		if targetFormationPreset[1].ActiveAbility.MaxCount > 0 then
			local replaceSlot = reorderedAbilityPreset[vSlotIndex]
			dc:UpdatePCProperty(roster, 'AbilityPreset/Preset/'..targetFormationPreset[2]..'/Order', replaceSlot[3]);
			vSlotIndex = vSlotIndex + 1;
			lastReplacedIndex = i;
		end
	end
	local baseSlot = reorderedAbilityPreset[fillUpIndex];
	dc:UpdatePCProperty(roster, 'AbilityPreset/Preset/'..baseSlot[2]..'/Order', lastReplacedIndex);
	
	dc:UpdatePCProperty(roster, 'AbilityPreset/Preset/'..slotIndex..'/ActiveAbility/MaxCount', 0);
	dc:UpdatePCProperty(roster, 'AbilityPreset/Preset/'..slotIndex..'/Title', '');
	return {Success = true};
end
...
function LobbyAction_InvokeQuestMission(dc, company, args)
	...
	if not questCls or not questCls.ReplayMission then
		return {Success = false};
	end
	StartMission(company, questCls.ReplayMission.name, { QuestType = questCls.name, TroubleBookEpisode = 'QuestReplay', ChallengerMode = args.ChallengerMode, QuestReplay = true });
	return {Success = true};
end
-- 무기 코스튬 변경
function LobbyAction_TransmogItem(dc, company, args)
	local itemAddress = args.ItemAddress;
	local item;
	if itemAddress == 'Inventory' then
		local itemInstanceKey = args.BaseInvKey;
	...
	if item == nil then
		LogAndPrint('[TransmogItem Failed] 대상 아이템을 찾지 못했다. company : ', company.name, ', : args : ', args);
		return {Success = false, Reward = {}};
	end
	local target = nil;
	if args.TransmogTarget and args.TransmogTarget ~= 'None' then
		target = GetClassList('Item')[args.TransmogTarget];
	end
	local isEnable, reason = IsEnableTransmogItem(company, item, target, LobbyInventoryItemCounter(company));
	if not isEnable then
		return {Success = false, Reward = {}};
	end
	-- 형상 변환
	dc:UpdateItemProperty(item, 'Transmog', target and target.name or 'None');
	if not item.Binded then
		dc:UpdateItemProperty(item, 'Binded', true);
	end
	-- 재료 뺏기
	local reward = {};
	local requireItem, requireCount = GetTransmogRequireItem(item, target);
	if requireItem and requireCount > 0 then
		dc:TakeItem(GetInventoryItemByType(company, requireItem.name), requireCount);
		table.insert(reward, {ItemName = requireItem.name, ItemCount = requireCount});
	end
...
function ProgressRetrainBeastMasteryAction(ldm, self, company, env, parsedScript)
	...
	for i, stageInfo in ipairs(targetStages) do
		...
		if #candidateMasteries < 4 then
			candidateMasteries = PickBeastUniqueMasteryCandidate(roster, GetBeastTypeFromEvolutionStage(beastTypeCls, stage), true, 4, true);
			for j = 1, 4 do
				dc:UpdatePCProperty(roster, string.format('EvolutionMasteryCandidate%d', j), candidateMasteries[j]);
			end
...
function ProgressRetrainMachineMasteryAction(ldm, self, company, env, parsedScript)
	local roster = GetRoster(company, parsedScript.RosterName);
	if roster == nil then
		return false;
	end
	...
	local candidateMasteries = {};
	for i = 1, 6 do
		local candidateName = SafeIndex(roster, 'CraftMasteryCandidate', i);
		if candidateName and candidateName ~= 'None' then
			table.insert(candidateMasteries, candidateName);
		end
	end
	...
	-- 후보 선택
	if #candidateMasteries < 6 then
		candidateMasteries = PickMachineUniqueMasteryCandidate(roster);
		for i = 1, 6 do
			dc:UpdatePCProperty(roster, string.format('CraftMasteryCandidate/%d', i), candidateMasteries[i] or 'None');
		end
		dc:Commit('ProgressRetrainMachineMasteryAction');
	end
	
	local id, ok, result = ldm:Dialog('ScoutMachineCraft', {CandidateMasteries = candidateMasteries, TargetKey = roster.RosterKey});
	
	dc:UpdatePCProperty(roster, 'CraftMastery', result.Mastery);
	for i = 1, 6 do
		dc:UpdatePCProperty(roster, string.format('CraftMasteryCandidate/%d', i), 'None');
	end
...
function LobbyAction_RetrainMachine(dc, company, args)
	local roster = GetRoster(company, args.Roster);
	if roster == nil or roster.RosterType ~= 'Machine' then
		LogAndPrint('LobbyAction_RetrainMachine : roster is nil!');
		return {Success = false, Item = 'None', Count = 0};
	end
	
	local retrainCandidate = roster.CraftMasteryCandidate[1];
	if retrainCandidate == 'None' then
		local itemCounter = LobbyInventoryItemCounter(company);
		local enable = IsEnableRetrainMachine(roster, itemCounter);
		if not enable then
			return {Success = false};
		end
		-- 1) 필요 아이템 소모
		dc:TakeItem(GetInventoryItemByType(company, 'Statement_Module'), 20);
		dc:Commit('LobbyAction_RetrainMachine');
	end
	
	StartLobbyDialog(company, 'RetrainMachine_EntryPoint', {roster_name = args.Roster});
	return {Success = true};
end
-- 유대 관계 형성
function LobbyAction_AddBestFriend(dc, company, args)
	local roster = GetRoster(company, args.Roster);
	---@cast roster class_Pc
	local roster2 = GetRoster(company, args.Roster2);
	---@cast roster2 class_Pc
	if roster == nil or roster2 == nil then
		LogAndPrint('LobbyAction_AddBestFriend : roster is nil!');
		return {Success = false};
	end
	
	local enable, reaon, enableGrade, prevGrade = IsEnableBestFriendGradeUp(roster, roster2);
	if not enable then
		return {Success = false};	
	end
	
	-- 1) 상호 간의 유대 관계 설정
	local pairList = { { roster, roster2 }, { roster2, roster } };
	for _, pair in ipairs(pairList) do
		local from = pair[1];
		local to = pair[2];
		local _, slotIndex = GetBestFriendSlot(from, to.name);
		local _, emptyIndex = GetBestFriendSlot(from, 'None');
		if slotIndex ~= nil then
			dc:UpdatePCProperty(from, string.format('BestFriend/%d/Grade', slotIndex), enableGrade);
		elseif emptyIndex ~= nil then
			dc:UpdatePCProperty(from, string.format('BestFriend/%d/Target', emptyIndex), to.name);
			dc:UpdatePCProperty(from, string.format('BestFriend/%d/Grade', emptyIndex), enableGrade);
		end
	end
	dc:Commit('LobbyAction_AddBestFriend');
	
	-- 2) 아르카나 탑
	local addPointList = {};
	local rosterList = GetAllRoster(company, 'Pc');
	for _, pc in ipairs({ roster, roster2 }) do
		IfHavePersonality(pc, 'Arcana_Tower', function(personality)
			if prevGrade == 0 and enableGrade > 0 then
				for _, from in ipairs(rosterList) do
					if from ~= pc then
						table.insert(addPointList, { From = from, To = pc, AddPoint = personality.ApplyAmount });
					end
				end
			end
		end);
	end
	if #addPointList > 0 then
		AddPCFriendshipPointMultiple(dc, addPointList, nil, true);
		dc:Commit('LobbyAction_AddBestFriend');
	end
	
	-- 3) 유대 관계 특성 선택
	StartLobbyDialog(company, 'AddBestFriendMastery_EntryPoint', {roster_name = args.Roster, roster_name2 = args.Roster2, prev_grade = prevGrade, next_grade = enableGrade});
	return {Success = true};
end
function ProgressAddBestFriendMasteryAction(ldm, self, company, env, parsedScript)
	local dc = ldm:GetDatabaseCommiter();

	local roster = GetRoster(company, parsedScript.RosterName);
	local roster2 = GetRoster(company, parsedScript.RosterName2);
	if roster == nil or roster2 == nil then
		return false;
	end
	local grade = parsedScript.Grade;
	if grade < 1 or grade > 3 then
		return false;
	end
	local ret = _ProgressAddBestFriendMasteryAction(ldm, dc, roster, roster2, grade, not env.set_friend_not_first, 'DialogAction:'..env._cur_dialog.name);
	env.set_friend_not_first = true;
	return ret;
end
function _ProgressAddBestFriendMasteryAction(ldm, dc, roster, roster2, grade, initialize, commitMsg)
	-- 진행상태 복원 편의를 위해 항상 순서를 맞춤
	if roster.SlotIndex > roster2.SlotIndex then
		local temp = roster;
		roster = roster2;
		roster2 = temp;
	end
	---@cast roster class_Pc
	-- 친구가 아닌데 이런 로직에 오면 안 된다.
	local _, slotIndex = GetBestFriendSlot(roster, roster2.name);
	if slotIndex == nil then
		return false;
	end	
	-- 마지막 진행상태 복원
	local candidateMasteries = {};
	for i = 1, 4 do
		local candidateName = GetWithoutError(roster, string.format('BestFriend/%d/MasteryCandidate%d', slotIndex, i));
		if candidateName and candidateName ~= 'None' then
			table.insert(candidateMasteries, candidateName);
		end
	end
	...
	-- 새로 뽑음
	if #candidateMasteries == 0 then
		candidateMasteries = GetBestFriendMasteryCandidate(roster, roster2, grade);
		for i = 1, 4 do
			dc:UpdatePCProperty(roster, string.format('BestFriend/%d/MasteryCandidate%d', slotIndex, i), candidateMasteries[i] or 'None');
		end
		dc:Commit('ProgressAddBestFriendMasteryAction');
	end
	
	local id, ok, result = ldm:Dialog('SetFriend', {Grade = grade, CandidateMasteries = candidateMasteries, TargetKey = roster.RosterKey, TargetKey2 = roster2.RosterKey, Initialize = initialize});
	...
	for _, pair in ipairs(pairList) do
		...
		local _, slotIndex = GetBestFriendSlot(from, to.name);
		if slotIndex ~= nil then
			dc:UpdatePCProperty(from, string.format('BestFriend/%d/Mastery%d', slotIndex, grade), result.Mastery);
		end
	end
	for i = 1, 4 do
		dc:UpdatePCProperty(roster, string.format('BestFriend/%d/MasteryCandidate%d', slotIndex, i), 'None');
	end

	local ret = dc:Commit(commitMsg);
	if not IsEnableBestFriendGradeUp(roster, roster2) then
		ldm:CloseDialog('SetFriend');
	end
	return ret;
end
-- 유대 관계 해제
function LobbyAction_RemoveBestFriend(dc, company, args)
	local roster = GetRoster(company, args.Roster);
	---@cast roster class_Pc
	local roster2 = GetRoster(company, args.Roster2);
	---@cast roster2 class_Pc
	if roster == nil or roster2 == nil then
		LogAndPrint('LobbyAction_RemoveBestFriend : roster is nil!');
		return {Success = false};
	end
	
	if not IsBestFriend(roster, roster2.name) then
		return {Success = false};	
	end
	...
	for _, pair in ipairs(pairList) do
		local from = pair[1];
		local to = pair[2];
		local slot, slotIndex = GetBestFriendSlot(from, to.name);
		if slot ~= nil and slotIndex ~= nil then
			dc:UpdatePCProperty(from, string.format('BestFriend/%d/Target', slotIndex), 'None');
			dc:UpdatePCProperty(from, string.format('BestFriend/%d/Grade', slotIndex), 0);
			for i = 1, 4 do
				local masteryName = GetWithoutError(slot, string.format('Mastery%d', i));
				if masteryName and masteryName ~= 'None' then
					dc:UpdatePCProperty(from, string.format('BestFriend/%d/Mastery%d', slotIndex, i), 'None');
				end
			end
		end
	end
	...
	-- 2) 상호 간의 우호도 감소
	local addPoint = -100000000; -- 우호도 감소 자체를
	-- 상호 간의 우호도 감소이므로 '정의'의 추가 감소를 막음
	AddPCFriendshipPointBySystem(dc, roster, roster2, addPoint);
	AddPCFriendshipPointBySystem(dc, roster2, roster, addPoint);
	
	dc:Commit('LobbyAction_RemoveBestFriend');
	...
	for _, pc in ipairs({ roster, roster2 }) do
		IfHavePersonality(pc, 'Arcana_Tower', function(personality)
			for _, from in ipairs(rosterList) do
				if from ~= pc then
					table.insert(addPointList, { From = from, To = pc, AddPoint = -1 * personality.ApplyAmount2 });
				end
			end
		end);
	end
...
-- 유대 관계 변경
function LobbyAction_ChangeBestFriend(dc, company, args)
	local roster = GetRoster(company, args.Roster);
	---@cast roster class_Pc
	local roster2 = GetRoster(company, args.Roster2);
	---@cast roster2 class_Pc
	if roster == nil or roster2 == nil then
		LogAndPrint('LobbyAction_ChangeBestFriend : roster is nil!');
		return {Success = false};
	end
	...
	local _, _, enableGrade, prevGrade = IsEnableBestFriendGradeUp(roster, roster2);
	local isEnable, _ = IsEnableRemoveBestFriend(roster, roster2);
	if isEnable or prevGrade == 0 then
		return {Success = false};
	end	
	...
	dc:Commit('LobbyAction_ChangeBestFriend');
	
	StartLobbyDialog(company, 'AddBestFriendMastery_EntryPoint', {roster_name = args.Roster, roster_name2 = args.Roster2, prev_grade = 0, next_grade = enableGrade});	
	return {Success = true};
end
------------------------------------------------------
-- 임대료 변경
------------------------------------------------------
function LobbyAction_ChangeOfficeRentType(dc, company, args)
	local rentIndex = args.RentIndex;
	if rentIndex == nil then
		LogAndPrint('LobbyAction_ChangeOfficeRentType : rentIndex is nil!');
		return {Success = false, Price = 0};
	end
	local officeType = GetClassList('Office')[company.Office];
	local rentType = GetWithoutError(officeType, 'MonthlyRent');
	if rentType == nil or rentIndex <= 0 or rentIndex > #rentType then
		LogAndPrint('LobbyAction_ChangeOfficeRentType : rentIndex is invalid!', rentIndex);
		return {Success = false, Price = 0};
	end
	local duration = rentType[rentIndex].ClearCount;
	if duration == company.OfficeRentDuration then
		LogAndPrint('LobbyAction_ChangeOfficeRentType : same duration!', rentIndex);
		return {Success = false, Price = 0};
	end
	local needVill = company.OfficeRentVill / company.OfficeRentDuration * company.OfficeRentCounter;
	if needVill > 0 and company.Vill < needVill then
		LogAndPrint('LobbyAction_ChangeOfficeRentType : not enough vill!', company.Vill, needVill);
		return {Success = false, Price = 0};
	end
	-- 기존 임대료 정산 (일반적인 임대료 지급이 아니므로, 다 초기화함)
	if needVill > 0 then
		dc:AddCompanyProperty(company, 'Vill', -needVill);
	end	
	dc:UpdateCompanyProperty(company, 'OfficeRentCounter', 0);
	dc:UpdateCompanyProperty(company, 'OfficeRentCountDelayCont', 0);
	-- 임대료 조건 변경
	dc:UpdateCompanyProperty(company, 'OfficeRentType', rentIndex);
...
-- 장비 슬롯 관리
------------------------------------------------------
function LobbyAction_ChangeEquipmentSlot(dc, company, args)
	local slotIndex = args.SlotIndex;
	if slotIndex == nil then
		LogAndPrint('LobbyAction_ChangeEquipmentSlot : slotIndex is nil!');
		return {Success = false};
	end
	local roster = GetRoster(company, args.Roster);
	if roster == nil then
		LogAndPrint('LobbyAction_ChangeEquipmentSlot : roster is nil!');
		return {Success = false};
	end
	
	-- 1) 장비 슬롯 사용 인덱스 변경
	local curSlotIndex = roster.EquipmentSlot.Index;
	if curSlotIndex == slotIndex then
		return {Success = false};
	end
	dc:UpdatePCProperty(roster, 'EquipmentSlot/Index', slotIndex);
	dc:Commit('LobbyAction_ChangeEquipmentSlot');
	
	-- 2) 특성으로 사라지는 장착 슬롯에 착용 중인 아이템이 있으면 해제
	local masteryTable = GetMastery(roster);
	local itemSlotExtentions = {'AlchemyBag', 'GrenadeBag', 'DoubleGear', 'NinjaToolkit', 'Module_AuxiliaryWeapon', 'Module_AssistEquipment'};
	for _, masteryName in ipairs(itemSlotExtentions) do
		if not GetMasteryMastered(masteryTable, masteryName) then
			local unequipItem = GetWithoutError(roster.Object, masteryName);
			if unequipItem and unequipItem.name  then
				dc:UnequipItem(roster, masteryName, slotIndex);
			end
		end
	end
...
function LobbyAction_AddEquipmentSlot(dc, company, args)
	local roster = GetRoster(company, args.Roster);
	if roster == nil then
		LogAndPrint('LobbyAction_AddEquipmentSlot : roster is nil!');
		return {Success = false};
	end 
	local itemCounter = LobbyInventoryItemCounter(company);
	local isEnable, needItem, needCount, reason = IsEnableAddEquipmentSlot(roster, itemCounter);
	if not isEnable then
		LogAndPrint('LobbyAction_AddEquipmentSlot : roster : ', args.Roster);
		for index, reason in ipairs (reason) do
			LogAndPrint('\nReason:', reason);
		end
		return {Success = false};
	...
	-- 기본값이 0부터 시작하지 않으므로, AddPCProperty를 사용하면 안 된다.
	dc:UpdatePCProperty(roster, 'EquipmentSlot/Count', roster.EquipmentSlot.Count + 1);
	if needItem ~= 'None' and needCount > 0 then
		dc:TakeItem(GetInventoryItemByType(company, needItem), needCount);
	end
	return {Success = true};
end
function LobbyAction_RenameEquipmentSlot(dc, company, args)
	local slotIndex = args.SlotIndex;
	if slotIndex == nil then
		LogAndPrint('LobbyAction_RenameEquipmentSlot : slotIndex is nil!');
		return {Success = false};
	end
	local roster = GetRoster(company, args.Roster);
	if roster == nil then
		LogAndPrint('LobbyAction_RenameEquipmentSlot : roster is nil!');
		return {Success = false};
	end
	if slotIndex >= roster.EquipmentSlot.RCount or slotIndex > #roster.EquipmentSlot.Slot then
		LogAndPrint('LobbyAction_RenameEquipmentSlot : slotIndex is invalid!');
		return {Success = false};
	end
	local propName = string.format('EquipmentSlot/Slot/%d/Title', slotIndex + 1);
	dc:UpdatePCProperty(roster, propName, args.SlotName);
	return {Success = true};
end
function LobbyAction_SwapEquipmentSlotOrder(dc, company, args)
	local slotIndex1 = args.SlotIndex;
	local slotIndex2 = args.SlotIndex2;
	if slotIndex1 == nil or slotIndex2 == nil then
		LogAndPrint('LobbyAction_MoveEquipmentSlot : slotIndex is nil!');
		return {Success = false};
	end
	local roster = GetRoster(company, args.Roster);
	if roster == nil then
		LogAndPrint('LobbyAction_MoveEquipmentSlot : roster is nil!');
		return {Success = false};
	end
	if slotIndex1 >= roster.EquipmentSlot.RCount or slotIndex1 > #roster.EquipmentSlot.Slot then
		LogAndPrint('LobbyAction_MoveEquipmentSlot : slotIndex1 is invalid!', slotIndex1);
		return {Success = false};
	end
	if slotIndex2 >= roster.EquipmentSlot.RCount or slotIndex2 > #roster.EquipmentSlot.Slot then
		LogAndPrint('LobbyAction_MoveEquipmentSlot : slotIndex2 is invalid!', slotIndex2);
		return {Success = false};
	end
	local order1 = roster.EquipmentSlot.Slot[slotIndex1 + 1].Order;
	local order2 = roster.EquipmentSlot.Slot[slotIndex2 + 1].Order;
	local propName1 = string.format('EquipmentSlot/Slot/%d/Order', slotIndex1 + 1);
	local propName2 = string.format('EquipmentSlot/Slot/%d/Order', slotIndex2 + 1);
	dc:UpdatePCProperty(roster, propName1, order2);
	dc:UpdatePCProperty(roster, propName2, order1);
	return {Success = true};
end
function LobbyAction_SwapRosterSlotOrder(dc, company, args)
	local r1 = GetRoster(company, args.Roster);
	local r2 = GetRoster(company, args.Roster2);

	if r1 == nil or r2 == nil or r1.RosterType ~= r2.RosterType then
		LogAndPrint(r1, r2, SafeIndex(r1, 'RosterType'), SafeIndex(r2, 'RosterType'));
		return {Success = false};
	end

	local s1 = GetRosterSlotIndex(r1);
	local s2 = GetRosterSlotIndex(r2);
	dc:UpdatePCProperty(r1, 'SlotIndex', s2);
	dc:UpdatePCProperty(r2, 'SlotIndex', s1);
	return {Success = true};
end
function LobbyAction_QueryChannelList(dc, company, args)
	return {Success = true, CurrentChannel = GetCurrentChannel(company), Channels = GetChannelList(args.LobbyType)};
end
function LobbyAction_QueryJointTrainingAvailability(dc, company, args)
	local targetCompany = GetCompanyByName(args.CompanyName);
	if targetCompany == nil then
		return {Success = false, AvailableModes = {}, AvailableRules = {}};
	end
	if not IsJointTrainingPVPOpened(targetCompany) or company.SecondWind.BreakLimit or targetCompany.SecondWind.BreakLimit then
		return {Success = true, AvailableModes = {}, AvailableRules = {}, BreakLimit = targetCompany.SecondWind.BreakLimit};
	end
	local ret = {Success = true};
	ret.AvailableModes = Linq.new(GetClassList('JointTrainingMode'))
		:where(function(m) return company.JointTrainingMode[m[1]].Opened and targetCompany.JointTrainingMode[m[1]].Opened end)
		:select(function(m) return m[1] end)
		:toList();
	local myRosters = GetAllRoster(company);
	local targetRosters = GetAllRoster(targetCompany);
	ret.AvailableRules = Linq.new(GetClassList('JointTrainingBanPickRule'))
		:where(function(m) return #myRosters >= m[2].MinRosterCount and #targetRosters >= m[2].MinRosterCount end)
		:select(function(m) return m[1] end)
		:toList();
	return ret;
end

function LobbyAction_ChangeOnlineState(dc, company, args)
	if not Set.new({'Online', 'AFK', 'Hidden'}):has(args.State) then
		return {Success = false};
	end
	dc:UpdateCompanyProperty(company, 'OnlineState', args.State);
	dc:Commit('ChangeOnlineState');
	RequestUpdateOnlineState(company, args.State);
	return {Success = true};
end

function LobbyAction_ChangeWhipserFirewall(dc, company, args)
	if not Set.new({ 'Open', 'Friend', 'Close' }):has(args.State) then
		return { Success = false };
	end
	dc:UpdateCompanyProperty(company, 'WhisperFirewall', args.State);
	return { Success = true };
end