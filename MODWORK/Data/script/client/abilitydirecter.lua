function MasterAbilityScript(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents, abilityAnimationInfo)
	...
	if not IsMoveTypeAbility(ability) then
		if IsClient() and not ad:GetConfig('NoCamera') and GetOption().Gameplay.DisableAbilityCam then
			if not ability.NoSystemCamera then
	...
	if ability.NoSystemCamera then
	...
	if ad:GetConfig('Preemptive') then
		...
		-- [MOD] 先制反击（ForestallFast）：多人同时对同一敌人触发时，
		-- 完全取消排队等待（原版压制射击按 PreemptiveOrder*2 秒错峰演出，
		-- 会让第N个单位多等 N*2 秒，造成数秒停顿）。先制反击是已触发后的
		-- 连射，直接 Sleep(0) 交给引擎自行调度齐射演出，不影响原版压制射击。
		local forestallFast = ad:GetConfig('ForestallFast');
		if forestallFast then
			ad:Wait(ad:Sleep(0));
		else
	...
	if ad:GetConfig('Preemptive') or (isAutoCameraTarget and not ad:GetConfig('NoCamera')) or ad:GetConfig('SystemCamera') then
		if not TestDisableTurnCamTarget(userInfo.User) or (userInfo.Target.name ~= nil and not TestDisableTurnCamTarget(userInfo.Target)) then
			ad:Wait(ad:Sleep(0));
		elseif not ability.AbilityWithMove then
			local camId;
			...
			ad:Wait(camId);
		elseif ability.AbilityWithSubRange then
			local camId;
			if userInfo.Target.name ~= nil and userInfo.Target ~= userInfo.User then
				camId = ad:ChangeCameraTargetingMode(GetObjKey(userInfo.User), GetObjKey(userInfo.Target), '_SYSTEM_', false, true, 0.5);
			else
	...
	elseif not ad:GetConfig('SystemCamera') and not IsMoveTypeAbility(ability) and TestDisableTurnCamTarget(userInfo.User) then
		ad:Wait(playID);
	...
	if achievementTest or IsClient() then
		...
		if #achievements > 0 then
			...
			for _, achievement in ipairs(achievements) do
				if achievementTest then
					...
					local achieveID = ad:UpdateAchievement(achievement, true);
		...
		if #addStats > 0 then
			...
			for _, addStat in ipairs(addStats) do
				if achievementTest then
					...
					local achieveID = ad:AddAchievementStat(addStat.Type, addStat.Value);
					ad:Connect(achieveID, prevID, -1);
					prevID = achieveID;
				end
			end
...
function GetAbilityAchievements(ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	...
	if not HasAchievement('SituationDoubleKO') and ability.Type == 'Attack' and ability.ApplyTarget == 'Any' then
	...
	if not HasAchievement('SituationDodgeInSmoke') and ability.Type == 'Attack' and relation == 'Enemy' then
	...
	if not HasAchievement('SituationWindPressureStun') and (ability.name == 'WindPressureGrenade' or ability.name == 'WindPressureGrenade_Uncommon' or ability.name == 'ShakeShot_Wind2') and relation == 'Team' then
	...
	if not HasAchievement('SituationPsionicStoneGet') and string.match(ability.name, '^InvestigatePsionicStone.*') then
		table.insert(addStats, {Type = 'PsionicStoneGetCount', Value = 1});
	end
	
	if not HasAchievement('SituationConceal10') and ability.name == 'Conceal' and relation == 'Team' then
		table.insert(addStats, {Type = 'ConcealCount', Value = 1});
	end
	
	if not HasAchievement('AbilityTrapDesigner') and ability.Type == 'Trap' and relation == 'Team' then
		table.insert(addStats, {Type = 'TrapUseCount', Value = 1});
	end
	
	if (ability.name == 'AnnihilatingFire' or ability.name == 'Machine_DoubleFire') and relation == 'Team' and not HasAchievement('AbilityHeissingAnnihilatingFire') then
		local enemyDeadCount = CheckTargetCountIf(primaryTargetInfos, secondaryTargetInfos, function(targetInfo)
			return GetRelationWithPlayer(targetInfo.Target) == 'Enemy' and targetInfo.IsDead;
		end);
	...
	for _, targetInfo in ipairs(infos) do
		if not HasAchievement('AbilitySionFlashAura') and targetInfo.Target.name ~= nil and targetInfo.Target.Info.name == 'Sion' and GetRelationWithPlayer(targetInfo.Target) == 'Team' then
		...
		--[[ TODO: 임시 봉인
		if not alreadySet.SituationDamage9999 and not HasAchievement('SituationDamage9999') and targetInfo.ShowDamage >= 9999 then
...
function GetUserAbilityAchievements_Albus(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if ability.name == 'GaleSlash' and not HasAchievement('AbilityAlbus') then
		local enemyDeadCount = CheckTargetCountIf(primaryTargetInfos, secondaryTargetInfos, function(targetInfo)
			return GetRelationWithPlayer(targetInfo.Target) == 'Enemy' and targetInfo.IsDead;
		end);
		if enemyDeadCount >= 3 then
	...
	if ability.name == 'Windwalker' and not HasAchievement('AbilityAlbusWindwalker') then	
	...
	if ability.name == 'StormSlash' and not HasAchievement('AbilityAlbusStormSlash') then
...
function GetUserAbilityAchievements_Sion(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if ability.name == 'LightningStorm' and not HasAchievement('AbilitySion') then
		local enemyDeadCount = CheckTargetCountIf(primaryTargetInfos, secondaryTargetInfos, function(targetInfo)
			return GetRelationWithPlayer(targetInfo.Target) == 'Enemy' and targetInfo.IsDead;
		end);
		if enemyDeadCount >= 3 then
	...
	if ability.name == 'FlashSpecialBeam' and not HasAchievement('AbilitySionFlashSpecialBeam') then
		local enemyDeadCount = CheckTargetCountIf(primaryTargetInfos, secondaryTargetInfos, function(targetInfo)
			return GetRelationWithPlayer(targetInfo.Target) == 'Enemy' and targetInfo.IsDead;
		end);
		if enemyDeadCount >= 3 then
...
function GetUserAbilityAchievements_Irene(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if not HasAchievement('AbilityIrene') then
	...
	if not HasAchievement('AbilityIreneUltimate4') then
		local isChainAbility, eventData = HasBattleEventType(battleEvents, 'ChainAbility');
		if isChainAbility and eventData.ChainCount == 4 then
			table.insert(achievements, 'AbilityIreneUltimate4');
		end
	end
	if ability.name == 'FlameStampKick' and not HasAchievement('AbilityIreneFlameStampKick') then
		local enemyDeadCount = CheckTargetCountIf(primaryTargetInfos, secondaryTargetInfos, function(targetInfo)
			return GetRelationWithPlayer(targetInfo.Target) == 'Enemy' and targetInfo.IsDead;
		end);
		if enemyDeadCount >= 3 then
...
function GetUserAbilityAchievements_Anne(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if ability.name == 'StarFall' and not HasAchievement('AbilityAnne') then
	...
	if ability.name == 'EntanglingRoots' and not HasAchievement('AbilityAnneEntanglingRoots') then
	...
	if ability.name == 'StarRain' and not HasAchievement('AbilityAnneStarRain') then
		local enemyDeadCount = CheckTargetCountIf(primaryTargetInfos, secondaryTargetInfos, function(targetInfo)
			return GetRelationWithPlayer(targetInfo.Target) == 'Enemy' and targetInfo.IsDead;
		end);
		if enemyDeadCount >= 3 then
...
function GetUserAbilityAchievements_Heissing(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if not HasAchievement('AbilityHeissing') then
...
function GetUserAbilityAchievements_Leton(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if not HasAchievement('AbilityLeton') then
	...
	if ability.name == 'FrostFinalKick' and not HasAchievement('AbilityLetonFrostFinalKick') then
		...
		for i, targetInfos in ipairs({primaryTargetInfos, secondaryTargetInfos}) do
			for j, targetInfo in ipairs(targetInfos) do
				...
				if target and target.name and GetRelationWithPlayer(target) == 'Enemy' then
					if targetInfo.IsDead then
						isDeadSet[targetKey] = true;
					end
					if targetInfo.PrevHP == targetInfo.MaxHP then
						isMaxHPSet[targetKey] = true;
					end
					if isDeadSet[targetKey] and isMaxHPSet[targetKey] then
						achieved = true;
						break;
					end
				end
...
function GetUserAbilityAchievements_Alisa(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if not HasAchievement('AbilityAlisa') then
...
function GetUserAbilityAchievements_Bianca(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if not HasAchievement('AbilityBianca') then
		local knockbackStunCount = CheckTargetCountIf(primaryTargetInfos, secondaryTargetInfos, function(targetInfo)
			return GetRelationWithPlayer(targetInfo.Target) == 'Enemy' and targetInfo.KnockbackPower > 0 and targetInfo.SlideType == 'None';
		end);
...
function GetUserAbilityAchievements_Giselle(achievements, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, battleEvents)
	if ability.Type == 'Attack' and not HasAchievement('AbilityGiselleHeadshot') then
	...
	if finalHit > -9999 then
		for i, action in ipairs(ret[finalHit]) do
			if IsAttackType(action.Type) then
				...
				if usingInfo.Target.name ~= nil and usingInfo.IsDead and not usingInfo.DeadByAfterShock then
...
function ProcessHit(ad, infoRoot, action, hitTimeActionID, relativeTime, hitArg, actionDB)
	...
	-- 데미지 표기
	if not ad:GetConfig('NoDamageShow') and not action.VirtualAction then
		local hitState = 'Hit';
	...
	-- 그림자 환영
	if hitArg.ShdowIllusion and action.FirstHit then
		ad:Connect(ad:UpdateBattleEvent(targetKey, 'BuffInvokedFromAbility', {Buff = 'ShdowIllusion'}), hitTimeActionID, relativeTime);
		ad:Connect(ad:PlayParticle(targetKey, '_BOTTOM_', 'Particles/Dandylion/Explosion_ShadowIllusion', 1, false), hitTimeActionID, relativeTime);
		ad:Connect(ad:HideObject(targetKey), hitTimeActionID, relativeTime);
	end
...
function ProcessHeal(ad, infoRoot, action, hitTimeActionID, relativeTime, hitArg, actionDB)
	...
	elseif action.HealType == 'Cost' then
		ad:Connect(ad:UpdateCostDamagedGauge(targetKey, {damage = hitArg.Damage, isFinal = action.FinalHit}), hitTimeActionID, relativeTime);
	elseif action.HealType == 'SP' then
		ad:Connect(ad:UpdateSPDamagedGauge(targetKey, {damage = hitArg.Damage, isFinal = action.FinalHit}), hitTimeActionID, relativeTime);
	end
...
function CalculateRangeType(usingInfo, rangeType)
	...
	elseif rangeType == 'AverageApplyPos' then
		...
		for _, pos in ipairs(applyRange) do
			posSum = table.map2(posSum, pos, __op_add);
		end 
...
function PlayLookAction(frame, infoRoot, action, mainAniID, ad, usingInfo, actionDB)
	local modelDirection = action.ModelDirection;
	local dir = CalculateDirection(usingInfo, modelDirection);	
	local actor = nil;
	if action.Target == 'User' then
	...
	dir = table.map2(GetPosition(actor), dir, __op_add);
	local retID = ad:LookPos(GetObjKey(actor), dir, false, true);
	ad:Connect(retID, mainAniID, frame / 30);
	return retID;
end
...
function PlayChangeShapeAction(frame, infoRoot, action, mainAniID, ad, usingInfo, actionDB)
	local actor = nil;
	if action.Target == 'User' then
		actor = usingInfo.User;
	elseif action.Target == 'Target' then
	...
	local ret = ad:ChangeShape(GetObjKey(actor), action.Shape, false);
	ad:Connect(ret, mainAniID, frame / 30);
	return ret;
end
function PlayCameraAction(frame, infoRoot, action, mainAniID, ad, usingInfo, actionDB)
	...
	local isEnablePlay = IsEnableTrigger(ad, usingInfo);
	local animSlope = action.CameraAnimSlope or 3;
	...
	if isEnablePlay then
		...
		if action.CameraMode == 'SelfToTarget' then
			cameraID = ad:ChangeCameraTargetingMode(user, target, action.CamTargetKey, StringToBool(action.DirectMove), StringToBool(action.ReleaseAfter, true), moveTime / 30, animSlope);
		elseif action.CameraMode == 'TargetToSelf' then
			cameraID = ad:ChangeCameraTargetingMode(target, user, action.CamTargetKey, StringToBool(action.DirectMove), StringToBool(action.ReleaseAfter, true), moveTime / 30, animSlope);
		elseif action.CameraMode == 'Self' then
			cameraID = ad:ChangeCameraTarget(user, action.CamTargetKey, StringToBool(action.DirectMove), StringToBool(action.ReleaseAfter, true), moveTime / 30, animSlope);
		elseif action.CameraMode == 'Target' then
			cameraID = ad:ChangeCameraTarget(target, action.CamTargetKey, StringToBool(action.DirectMove), StringToBool(action.ReleaseAfter, true), moveTime / 30, animSlope);
		elseif action.CameraMode == 'SelfToUsingPos' then
			local usingPos = { x = usingInfo.UsingPos.x, y = usingInfo.UsingPos.y, z = usingInfo.UsingPos.z};
			cameraID = ad:ChangeCameraTargetingPositionMode(user, usingPos, action.CamTargetKey, StringToBool(action.DirectMove), StringToBool(action.ReleaseAfter, true), moveTime / 30, animSlope);
		end
		if cameraID == -1 then
...
function BuildHitProcessArg(action, targetInfo)
	...
	-- 그림자 환영
	local damageFlag = GetWithoutError(targetInfo, 'DamageFlag');
	if targetInfo.Target and SafeIndex(damageFlag, 'ShdowIllusion') then
		hitProcessArg.ShdowIllusion = true
	end
...
function PlayForceHealAction(frame, infoRoot, action, mainAniID, ad, targetInfo, actionDB)
	local hitProcessArg = {};
	hitProcessArg.Target = targetInfo.Target;
	hitProcessArg.AttackerState = targetInfo.AttackerState;
	hitProcessArg.HitMethod = action.HitMethod;
	...
	if targetInfo.Target.name then
		return ProcessHeal(ad, infoRoot, action, forceID, -1, hitProcessArg, actionDB);
	else
		return forceID, actionDB;
	end
end
...
function PlayRushAction(frame, infoRoot, action, mainAniID, ad, usingInfo, actionDB)
	...
	---@type string|nil
	local lookTargetType = action.LookTarget or 'Target';
	local lookTarget;
	if lookTargetType == 'User' then
		lookTarget = usingInfo.User;
	elseif lookTargetType == 'Target' then
	...
	local speed = 0;
	if action.Speed ~= nil and action.Speed ~= 0 then
		useFixSpeed = true;
		speed = action.Speed;
	end
...
function IsSkipHideAction(action, target)
	-- 은신 연출은 아군이면, 숨기는 연출 생략 (Detected 상태는 일단 사라진 후, 바로 발각되는 쪽으로 처리함)
	if action.Cloaking == 'true' then
		local relation = GetRelationWithPlayer(target);
...
function PlayVisibleAction(frame, infoRoot, action, mainAniID, ad, usingInfo, actionDB)
	...
	if action.Visible == 'true' then
		...
		if not IsSkipHideAction(action, targetUnit) then
...
function PlayWeaponVisibleAction(frame, infoRoot, action, mainAniID, ad, usingInfo, actionDB)
	local targetUnit = action.Target == 'User' and usingInfo.User or usingInfo.Target;
	local unitKey = GetObjKey(targetUnit);
	local actionId;
	if action.Visible == 'true' then
		actionId = ad:ShowWeapon(unitKey);
	else
		if not IsSkipHideAction(action, targetUnit) then
			actionId = ad:HideWeapon(unitKey);
		else
			actionId = ad:Sleep(0);
		end
	end
...
function PlayAction(frame, infoRoot, action, refID, ad, usingInfo, actionDB)
	local ActionPlayer = {
		...
		WeaponVisible = PlayWeaponVisibleAction,
		...
		ChangeShape = PlayChangeShapeAction,
		Look = PlayLookAction,
...
function PlayAbilityDirectInternal(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	...
	if abilityAnimationInfo.ReleasePose then
		...
		if ability.Animation ~= '' then
			local fadeOut = true;
			if ability.NoPrepare or ability.NoFadeOut then
				fadeOut = false;
			end
			mainAniID = ad:PlayAni(user, ability.Animation, false, -1, false, fadeOut);
		else
	...
	for _, action in ipairs(abilityAnimationInfo.Actions.MultiActionDefiner) do
		local updateFunc = loadstring(action.ActionModifier or '');
	...
	local damageAction = Set.new({'Hit', 'ForceHit', 'Heal', 'ForceHeal', 'ChainForceHit'});
	...
	local needVirtualHit = math.max(ability.RandomPickCount, 1) > #primaryTargetInfos;
	local sandbackMode = false
	local prevTargetDead = false;
	if needVirtualHit and ability.VirtualFinish then
		prevTargetDead = primaryTargetInfos[#primaryTargetInfos].IsDead;
		primaryTargetInfos[#primaryTargetInfos].IsDead = false;
		sandbackMode = true;
		userInfo.AttackerState = 'Critical';
		userInfo.DefenderState = 'Hit';
		userInfo.MainDamage = 0;
		userInfo.ShowDamage = 0;
	end
	...
	if needVirtualHit and ability.VirtualFinish then
		primaryTargetInfos[#primaryTargetInfos].IsDead = prevTargetDead;
	end
	local prevUserDead = userInfo.IsDead;
	local virtualTargetCount = math.max(ability.RandomPickCount, 1);
	for i = #primaryTargetInfos + 1, virtualTargetCount do
		if i == virtualTargetCount and sandbackMode then
			userInfo.IsDead = true;
		end
		for _, actionSet in ipairs(CollectActionFunc('Primary', userInfo, i + 1, sandbackMode)) do
			actionSet.Action.VirtualAction = true;
			if sandbackMode or actionSet.Action.AlternativeShot then
				table.insert(actionCollection, actionSet);
			end
		end
	end
	userInfo.IsDead = prevUserDead;
	--[[
	...
	for i, targetInfo in ipairs(primaryTargetInfos) do
		...
		if SafeIndex(targetInfo.DamageFlag, 'ShdowIllusion') then
			ad:Connect(ad:ShowObject(targetKey), endID, 0);			
		end
	end
...
--		local lastID = ProcessAbilityDamageFeedback(ad, userInfo, endID);
		return ProcessAbilityAttackVoice(ad, userInfo, primaryTargetInfos, secondaryTargetInfos, endID);
	else
		return endID;
	end
end
...
function PLAY_Move(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos)
	...
	if GetRelationWithPlayer(userInfo.User) == 'Team' 
		...
		and (not GetOption().Gameplay.DisableRushCam and not (GetOption().Gameplay.DisableTurnCam > 0))
...
function PLAY_StartOverwatch(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos)
	local user = GetObjKey(userInfo.User);
	local mainAni = ad:PlayAni(user, ability.Animation, false, -1, false, not ability.NoPrepare);
...
function Play_Communion(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos)
	local targetKey = GetObjKey(userInfo.User);
	if IsObjectInSight(userInfo.User) then
		ad:Sleep(0.1);
	end
	ad:PlayPose(targetKey, 'TameStart', 'TameStd', false, 'TameStdIdle');
	local soundID = ad:PlaySound3D('Characters/Crammy_Communion.wav', targetKey, '_CENTER_', 3000, 'Effect', 1.0);
	local particleID = ad:PlayParticle(targetKey, '_BOTTOM_', 'Particles/Dandylion/Buff_Conceal_Start', 1.5);
	ad:Connect(soundID, particleID, 0);
end
function Play_Occupy(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos)
	local targetKey = GetObjKey(userInfo.User);
	if IsObjectInSight(userInfo.User) then
		ad:Sleep(0.1);
	end
	ad:PlayPose(targetKey, 'InteractionStart', 'InteractionStd', false, 'InteractionStdIdle');
	local soundID = ad:PlaySound3D('Characters/Crammy_Communion.wav', targetKey, '_CENTER_', 3000, 'Effect', 1.0);
	local particleID = ad:PlayParticle(targetKey, '_BOTTOM_', 'Particles/Dandylion/Buff_Conceal_Start', 1.5);
	ad:Connect(soundID, particleID, 0);
end
...
function Play_Steal(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos)
	if IsObjectInSight(userInfo.User) then
		ad:Sleep(0.1);
	end
	local targetKey = GetObjKey(userInfo.User);
	local hide = ad:HideWeapon(targetKey);
	local playAni = ad:PlayAni(targetKey, ability.Animation, false, -1, false);
	local soundID = ad:PlaySound3D('Conceal.wav', targetKey, '_CENTER_', 3000, 'Effect', 1.0, true);
	local particleID = ad:PlayParticle(targetKey, '_BOTTOM_', 'Particles/Dandylion/Buff_Conceal_Start', 1);
	ad:Connect(particleID, playAni, 0);
	ad:Connect(soundID, particleID, 0);
	ad:Connect(ad:ShowWeapon(targetKey), playAni, -1);
end
...
function Play_SummonBeast(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	...
	local playSoundName = 'Cast/Cast_Whistle1.wav';
	local userKey = GetObjKey(userInfo.User);
	local ani = ad:PlayAni(userKey, ability.Animation, false, -1, false, not ability.NoPrepare);
	local playsoundID = ad:PlaySound3D(playSoundName, userKey, 'Chest', 3000, 'Voice', 2.0, true);
	local spawn = ad:Move(GetObjKey(userInfo.Target), up, true);
	ad:Connect(playsoundID, ani, 1.2);
	ad:Connect(spawn, ani, 2.1);
...
function Play_UnsummonBeast(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	...
	local userKey = GetObjKey(userInfo.User);
	local playSoundName = 'Cast/Cast_Whistle2.wav';
	local ani = ad:PlayAni(userKey, ability.Animation, false, -1, false, not ability.NoPrepare);
	local playsoundID = ad:PlaySound3D(playSoundName, userKey, 'Chest', 3000, 'Voice', 2.0, true);
	local move = ad:Move(GetObjKey(userInfo.Target), up, false);
	ad:Connect(playsoundID, ani, 1.2);
	ad:Connect(move, ani, 2.1);
	ad:Connect(ad:Move(GetObjKey(userInfo.Target), InvalidPosition(), true), move, 1);
end
function Play_CallRejoin(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	ad:ChangeCameraPosition(userInfo.UsingPos.x, userInfo.UsingPos.y, userInfo.UsingPos.z, false, 1);
end
...
function Play_HackingProtocol(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	...
	local commandAbility = GetAbilityObject(userInfo.User, protocolCls.Ability.name) or protocolCls.Ability;
...
function Play_SubCommand(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	local subCommand = GetClassList(ability.AbilitySubMenu)[userInfo.SubCommand];
	if subCommand == nil then
		return;
	end
	local commandAbility = GetAbilityObject(userInfo.User, subCommand.Ability.name) or subCommand.Ability;
	return PlayAbilityDirect(ad, commandAbility, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo);
end
...
function Play_BeholderVision(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	if IsObjectInSight(userInfo.User) then
		ad:Sleep(0.1);
	end
	local user = userInfo.User;
	local usingBeholderVisioin2 = GetBuff(user, 'UsingBeholderVision2');
	if usingBeholderVisioin2 then
		user = GetUnit(usingBeholderVisioin2.ReferenceTarget) or user;
	end
	local ownerKey = GetObjKey(user);
	ad:ChangeCameraTarget(ownerKey, '_SYSTEM_', false);
	for i, info in ipairs(primaryTargetInfos) do
		local targetKey = GetObjKey(info.Target);
		ad:ForceEffect(ownerKey, '_CENTER_', targetKey, '_CENTER_', 'VindictiveSpirit');	
	end	
end
function Play_ThrowerFlame(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo)
	local host = ability;
	if not IsClass(ability) then
		host = GetHostClass(ability);
	end
	if ability.RangeDistance == host.RangeDistance then
		PlayAbilityDirect(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo);
		return;
	end
	local animationInfoList = GetClassList('ObjectAnimationInfo');
	...
	abilityAnimationInfo = ClassToTable(abilityAnimationInfo);
	-- Force 이펙트의 거리 조절 (TargetPos는 방향만 정하고, 거리는 Distance 값을 사용하는 이펙트라서 추가 처리가 필요함)
	local forceRangeAction = abilityAnimationInfo.Actions.ForceRange[1];
	if forceRangeAction then
		local attbPairs = SafeIndex(forceRangeAction, 'ParticleAttribute', 'Pair') or {};
		-- 해당 이펙트의 기본값인 9에 해당 어빌리티의 현재 사거리와 기본 사거리의 비율을 곱해줌
		local distanceValue = 9 * ability.RangeDistance / host.RangeDistance;
		table.insert(attbPairs, { ParticleAttributeValue = { Type = 'Float', Value = distanceValue }, Key = 'Distance' });
		ForceNewIndex(forceRangeAction, 'ParticleAttribute', 'Pair', attbPairs);
	end
	-- Hit 개수와 타이밍 조절
	for _, action in ipairs(abilityAnimationInfo.Actions.MultiActionDefiner) do
		local frameSize = #action.FrameList.Frame;
		local first = action.FrameList.Frame[1].Frame;
		local last = action.FrameList.Frame[frameSize].Frame;

		local newFrameList = {};
		local newFrameSize = math.floor(ability.RangeDistance);
		for i = 1, newFrameSize do
			local newFrame = math.floor(math.lerp(first, last, i / newFrameSize));
			table.insert(newFrameList, { Frame = newFrame });
		end
		action.FrameList.Frame = newFrameList;
	end
	PlayAbilityDirect(ad, ability, userInfo, primaryTargetInfos, secondaryTargetInfos, abilityAnimationInfo);
end
----------------------------------------- AbilityDirect Check&Get Data Utility  -----------------------------------------------
function IsAttackType(actionType)
	return actionType == 'Hit' or actionType == 'ForceHit' or actionType == 'ChainForceHit' or actionType == 'Heal' or actionType == 'ForceHeal';
end
